...
def plot_share_of_vehicle_type_by_transport_type(ECONOMY_IDs,new_sales_shares_all_plot_drive_shares_df,stocks_df,fig_dict, color_preparation_list, colors_dict, share_of_transport_type_type, INCLUDE_GENERAL_DRIVE_TYPES=False):
    PLOTTED=True
        ...
        for economy in ECONOMY_IDs:
            ...
            if transport_type=='passenger':
                ...
                if WRITE_HTML:
                    ...
                    # energy_use_by_fuel_type_economy = energy_use_by_fuel_type_economy.groupby(['Economy', 'Date', 'Fuel','Unit']).filter(lambda x: x['Energy'].sum() > 0.001*energy_use_by_fuel_type_economy['Energy'].sum())
                    ...
                    #now plot
                    title_text = f'Passenger road energy by fuel - {scenario}'
                    fig = px.area(energy_use_by_fuel_type_economy.loc[energy_use_by_fuel_type_economy['Transport Type']=='passenger'], x='Date', y='Energy', color='Fuel', title=title_text, color_discrete_map=colors_dict)
                    #add y axis title
                    fig.update_yaxes(title_text='PJ')
                    #make text bigger
                    fig.update_layout(font_size=35)
                    fig.write_html(f'./plotting_output/dashboards/{economy}/transport_type_intensity_analysis/energy_use_by_fuel_type_passenger.html')
...
def plot_share_of_vehicle_type_activity(ECONOMY_IDs,model_output_detailed_df,fig_dict,  color_preparation_list, colors_dict, transport_type):
    PLOTTED=True
    #loop through scenarios and grab the data for each scenario:
    #create a new df with only the data we need:
    share_of_activity = model_output_detailed_df.copy()
    #drop where medium isnt road
...
def freight_tonne_km_by_drive(ECONOMY_IDs,model_output_detailed,fig_dict,  color_preparation_list, colors_dict, medium):
    PLOTTED=True
...
def passenger_km_by_drive(ECONOMY_IDs,model_output_detailed,fig_dict,  color_preparation_list, colors_dict, medium):
    PLOTTED=True
...
def activity_growth(ECONOMY_IDs,model_output_detailed_df,fig_dict,  color_preparation_list, colors_dict, RECALCULATE=True, SMOOTH=False):
    PLOTTED=True
    ...
    if RECALCULATE:
        ...
        #save to x.scv
        # model_output_detailed.to_csv('model_output_detailed.csv', index=False)
...
def macro_lines(ECONOMY_IDs, growth_forecasts, fig_dict, color_preparation_list, colors_dict, measure, indexed=False):
...
def activity_and_macro_growth_lines(ECONOMY_IDs,original_model_output_8th_df,model_output_detailed,growth_forecasts, fig_dict, color_preparation_list, colors_dict, indexed=False):
...
def plot_demand_side_fuel_mixing(ECONOMY_IDs,demand_side_fuel_mixing_df,model_output_detailed_df,fig_dict, color_preparation_list, colors_dict):
    PLOTTED=True
...
def create_charging_plot(ECONOMY_IDs,chargers_df,fig_dict, color_preparation_list, colors_dict, WRITE_HTML=False):
    PLOTTED=True 
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        for economy in ECONOMY_IDs:
            ...
            if WRITE_HTML:
                ...
                title = 'Expected slow and fast public chargers needed for ' + scenario + ' scenario'
                fig_ = px.bar(chargers_economy, x="Date", y=['Fast chargers (200kW)','Slow chargers (60kW)'], title=title, color_discrete_map=colors_dict)

                #add units to y col
                title_text = '(thousands)'
                fig_.update_yaxes(title_text=title_text)#not working for some reason

                fig_.update_layout(font_size=30)   
                #make bar thicker
                fig_.update_traces(marker_line_width=2.5)
                #make title the scenaro
                fig_.update_layout(title=f'Public chargers - {scenario}')    
                #remove legend title
                fig_.update_layout(legend_title_text='')              
                fig_.write_html(f'./plotting_output/dashboards/{economy}/transport_type_intensity_analysis/charging.html')
            
    #put labels for the color parameter in color_preparation_list so we can match them against suitable colors:
    color_preparation_list.append(['sum_of_fast_chargers_needed','sum_of_slow_chargers_needed'])
    return fig_dict, color_preparation_list

def prodcue_LMDI_mutliplicative_plot(ECONOMY_IDs,fig_dict, colors_dict, transport_type, medium):
    PLOTTED=True
    for scenario in config.economy_scenario_concordance.Scenario.unique():
        for economy in ECONOMY_IDs:
            if medium == 'all': 
...
def produce_LMDI_additive_plot(ECONOMY_IDs,fig_dict, colors_dict, medium):
    PLOTTED=True
    for scenario in config.economy_scenario_concordance.Scenario.unique():
        for economy in ECONOMY_IDs:
            if medium == 'all': 
...
def plot_stocks_per_capita(ECONOMY_IDs,gompertz_parameters_df, model_output_detailed, first_road_model_run_data, fig_dict, color_preparation_list, colors_dict, PLOT_ANYWAY=True):
    PLOTTED=True
    ...
    ECONOMIES_WITH_MAX_STOCKS_PER_CAPITA_REACHED = yaml.load(open('config/parameters.yml'), Loader=yaml.FullLoader)['ECONOMIES_WITH_MAX_STOCKS_PER_CAPITA_REACHED']
    ...
    # DO_LOG_FITTING_ON_ONLY_PASSENGER_VEHICLES_DICT = yaml.load(open('config/parameters.yml'), Loader=yaml.FullLoader)['DO_LOG_FITTING_ON_ONLY_PASSENGER_VEHICLES_ECONOMIES']
    # if DO_LOG_FITTING_ON_ONLY_PASSENGER_VEHICLES_DICT[ECONOMY_IDs[0]]:
    #     vehicles_per_stock_parameters = pd.read_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters_passenger_only.csv'.format(ECONOMY_IDs[0]))
    ...
    # else:
    vehicles_per_stock_parameters = pd.read_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters.csv'.format(ECONOMY_IDs[0]))
...
def plot_non_road_energy_use(ECONOMY_IDs,energy_output_for_outlook_data_system_tall, fig_dict,  color_preparation_list, colors_dict,transport_type,WRITE_HTML=False):
    PLOTTED=True
        ...
        for economy in ECONOMY_IDs:
            ...
            if transport_type=='passenger':
                ...
                if WRITE_HTML:
                    ...
                    #make title the scenaro
                    fig.update_layout(title=f'Non road energy by Fuel - {scenario}')    
                    #remove legend title
                    fig.update_layout(legend_title_text='')     
                    #put  (PJ) in the y axis
                    fig.update_yaxes(title_text='PJ')#not working for some reason         
                    fig.write_html(f'./plotting_output/dashboards/{economy}/transport_type_intensity_analysis/energy_use_by_fuel_type_non_road.html')
            else:
                raise ValueError('transport_type must be passenger, all or freight')
...
def road_stocks_by_drive_type(ECONOMY_IDs,model_output_detailed_df, fig_dict, color_preparation_list, colors_dict,transport_type):
    PLOTTED=True
...
def road_sales_by_drive_vehicle(ECONOMY_IDs,model_output_detailed_df, fig_dict, color_preparation_list, colors_dict):
    PLOTTED=True
    #break activity into its ddrive, transport and "Vehicle Type" and plot as a line chart. will do a plot for each transport type or a plot where passneger km and freight km are in same plot.
    # model_output_detailed.pkl
    #loop through scenarios and grab the data for each scenario:
...
def turnover_rate_by_vehicle_type_line(ECONOMY_IDs,model_output_detailed,fig_dict, color_preparation_list, colors_dict,transport_type, medium):
    PLOTTED=True
...
def emissions_by_fuel_type(ECONOMY_IDs, emissions_factors,model_output_with_fuels_df, fig_dict, color_preparation_list, colors_dict,transport_type, USE_AVG_GENERATION_EMISSIONS_FACTOR=True, USE_CUM_SUM_OF_EMISSIONS=False):
    PLOTTED=True
    ...
    if USE_AVG_GENERATION_EMISSIONS_FACTOR:
        ...
        emissions_factor_elec = pd.read_csv('input_data/from_8th/outlook_8th_emissions_factors_with_electricity.csv')#c:\Users\finbar.maunsell\github\aperc-emissions\output_data\outlook_8th_emissions_factors_with_electricity.csv
...
def plot_comparison_of_energy_by_dataset(ECONOMY_IDs,energy_output_for_outlook_data_system_df, bunkers_data_df, energy_use_esto,esto_bunkers_data, energy_8th, fig_dict, color_preparation_list, colors_dict, mapping_type, INCLUDE_8TH=False, INCLUDE_BUNKERS=False,ONLY_BUNKERS=False):
...
def plot_comparison_of_energy_to_previous_9th_projection(ECONOMY_IDs,energy_output_for_outlook_data_system_df, bunkers_data_df, previous_projection_df, previous_bunkers_data_df, previous_projection_date_id, fig_dict, color_preparation_list, colors_dict, mapping_type, id_string, energy_use_esto, esto_bunkers_data, energy_8th, INCLUDE_BUNKERS, ONLY_BUNKERS):
...
def plot_pct_comparison_of_energy_compared_to_8th(ECONOMY_IDs,energy_output_for_outlook_data_system_df, bunkers_data_df, energy_8th, fig_dict, color_preparation_list, colors_dict, mapping_type, measure,INCLUDE_BUNKERS):
...
def plot_energy_efficiency_timeseries(ECONOMY_IDs,model_output_detailed,fig_dict,DROP_NON_ROAD_TRANSPORT, color_preparation_list, colors_dict, transport_type, extra_ice_line=True,extra_bev_line=True, vehicle_type_grouping='simplified', WRITE_HTML=True):
    PLOTTED=True
    conversion_factors = pd.read_csv('config/concordances_and_config_data/conversion_factors.csv')
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        for economy in ECONOMY_IDs:
            ...
            if WRITE_HTML:
                ...
                # energy_eff_by_scen_by_economy.to_csv(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/{scenario}_{economy}_energy_efficiency_timeseries_{transport_type}_no_HEVs.csv')
                #load the df from the csv and merge it but label it as 'no HEV's'
                # energy_eff_by_scen_by_economy_no_HEVS = pd.read_csv(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/{scenario}_{economy}_energy_efficiency_timeseries_{transport_type}_no_HEVs.csv')
                ...
                title = 'Energy efficiency - {} - {}'.format(scenario, economy)
                fig = px.line(energy_eff_by_scen_by_economy, x='Date', y='Efficiency', color='ICE_ONLY', title=title, color_discrete_map=colors_dict)#, line_dash='Vehicle Type')    #,line_dash='line_type'
                #add y axis:
                fig.update_yaxes(title='km per MJ')
                ...
                title=dict(text='')))
                fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/{scenario}_{economy}_energy_efficiency_timeseries_{transport_type}.html')
...
def plot_energy_efficiency_strip(ECONOMY_IDs,model_output_detailed,fig_dict,DROP_NON_ROAD_TRANSPORT, color_preparation_list, colors_dict):
    PLOTTED=True
...
def plot_energy_intensity_strip(ECONOMY_IDs,model_output_detailed_detailed_non_road_drives,fig_dict, color_preparation_list, colors_dict):
    PLOTTED=True
...
def plot_energy_intensity_timeseries(ECONOMY_IDs,model_output_detailed_detailed_non_road_drives,fig_dict, color_preparation_list, colors_dict, medium):
...
def sales_and_turnover_lines(ECONOMY_IDs,model_output_detailed_df,fig_dict, color_preparation_list, colors_dict, transport_type):
    PLOTTED=True
...
def plot_turnover_rate_age_curve(ECONOMY_IDs,model_output_detailed_df,fig_dict, colors_dict):
    PLOTTED=True
...
def plot_mileage_timeseries(ECONOMY_IDs,model_output_detailed,fig_dict, color_preparation_list, colors_dict, transport_type):
    PLOTTED=True
...
def plot_mileage_strip(ECONOMY_IDs,model_output_detailed,fig_dict, color_preparation_list, colors_dict):
    PLOTTED=True
    #to help with checking that the data is realistic, plot energy efficiency here:
...
def compare_8th_and_9th_stocks_sales(ECONOMY_IDs,data_8th, model_output_detailed,fig_dict, color_preparation_list, colors_dict, measure):
...
def plot_age_distributions(ECONOMY_IDs,model_output_detailed_detailed_non_road_drives,fig_dict, color_preparation_list, colors_dict, medium, BY_DRIVE, BY_VEHICLE_TYPE):
...
def line_energy_use_by_transport_type(ECONOMY_IDs,model_output_detailed,fig_dict,medium, color_preparation_list, colors_dict, transport_type):
        ...
        for economy in ECONOMY_IDs:
            ...
            if share_of_transport_type_type == 'passenger':
                ...
                if CREATE_INDIVIDUAL_HTML_FILES:   
                    ...
                    fig.update_traces(line=dict(width=10))
                    #make title the scenaro
                    fig.update_layout(title='Light private vehicle shares - {} - {}'.format(economy, scenario))
                    fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/INTENSITY_ANALYSIS_sales_share_by_transport_type_passenger_{scenario}.html')
                #add fig to dictionary for scenario and economy:
                ...
                if CREATE_INDIVIDUAL_HTML_FILES:  
                    ...
                    #make line thicker
                    fig.update_traces(line=dict(width=10))
                    fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/INTENSITY_ANALYSIS_sales_share_by_transport_type_freight.html')
                ...
                if CREATE_INDIVIDUAL_HTML_FILES:    
                    #make line thicker
                    fig.update_traces(line=dict(width=10))
                    fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/INTENSITY_ANALYSIS_sales_share_by_transport_type_all.html')
                #add fig to dictionary for scenario and economy:
    ...
    for scenario in lca['Scenario'].unique():
        ...
        for economy in lca_scenario['Economy'].unique():
            ...
            if CREATE_SEPARATE_HTMLS:
                ...
                #make lines slightly thicker
                fig.update_traces(line=dict(width=10))
                fig.write_html(f'plotting_output/lifecycle_emissions/lifecycle_emissions_of_cars_{scenario}_{economy}.html')
            #add fig to dictionary for scenario and economy:
...
def plot_lca_inputs(lca_inputs):
    ...
    fig.write_html('plotting_output/lifecycle_emissions/lifecycle_emissions_of_cars_inputs.html')
...
def plot_emissions_from_use_for_single_vehicle(emissions_factors, fuels_to_plot):   
    ...
    for scenario in emissions_factors_filtered.Scenario.unique():
        ...
        #drop the legend
        fig.update_layout(showlegend=False)
        fig.write_html(f'plotting_output/lifecycle_emissions/{economy}_lifecycle_emissions_LINE_{scenario}.html')
...
def extract_lifecycle_emissions_series():
    ...
    lca = pd.read_excel('input_data/lifecycle_emissions.xlsx')
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        for economy in ECONOMY_IDs:
            ...
            if WRITE_HTML:
                ...
                if 'lpv' in energy_eff_by_scen_by_economy['Vehicle Type'].unique():
                    energy_eff_by_scen_by_economy = energy_eff_by_scen_by_economy.loc[energy_eff_by_scen_by_economy['Vehicle Type']=='lpv'].copy()
                fig = px.line(energy_eff_by_scen_by_economy, x='Date', y='New_vehicle_efficiency', color='ICE_ONLY',title=title, color_discrete_map=colors_dict)#, line_dash='Vehicle Type')line_dash='ICE_ONLY', 
                ...
                fig.update_layout(legend=dict(
                title=dict(text='')))
                fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/{scenario}_{economy}_new_vehicle_efficiency_timeseries_{transport_type}.html')
    #put labels for the color parameter in color_preparation_list so we can match them against suitable colors:
    color_preparation_list.append(energy_eff_by_scen_by_economy['Vehicle Type'].unique().tolist())
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        for economy in ECONOMY_IDs:
            ...
            if WRITE_HTML:
                ...
                #drop legend
                fig.update_layout(showlegend=False)
                fig.write_html(f'plotting_output/dashboards/{economy}/transport_type_intensity_analysis/{scenario}_{economy}_new_vehicle_emissions_intensity_timeseries_{transport_type}.html')
    #put labels for the color parameter in color_preparation_list so we can match them against suitable colors:
...
def plot_number_of_stocks_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, model_output_detailed_df, colors_dict, transport_type, SALES,SUM_OF_ALL_ECONOMIES):
    ...
    for scenario in stocks['Scenario'].unique():
        ...
        if transport_type == 'passenger':
            ...
            #save to html
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/{sales_or_stocks}_{scenario}_{economy_grouping_name}_passenger.html')
        elif transport_type == 'freight':
            ...
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/{sales_or_stocks}_{scenario}_{economy_grouping_name}_freight.html')
            ...
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/{sales_or_stocks}_{scenario}_{economy_grouping_name}_all.html')
        else:
...
def plot_share_of_vehicle_type_by_transport_type_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, new_sales_shares_all_plot_drive_shares_df,stocks_df, colors_dict, share_of_transport_type_type):
    ...
    for scenario in new_sales_shares_all_plot_drive_shares['Scenario'].unique():
        ...
        if share_of_transport_type_type == 'passenger':
            ...
            #save to html
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_passenger.html')
        elif share_of_transport_type_type == 'freight':
            ...
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_freight.html')
            ...
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_all.html')
        else:
            raise ValueError('share_of_transport_type_type must be either passenger or freight')
    return
...
def plot_supply_side_fuel_mixing_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, supply_side_fuel_mixing_df, colors_dict):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        #save to html
        fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/supply_side_fuel_mixing_{scenario}_{economy_grouping_name}.html')
...
def energy_use_by_fuel_type_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, energy_output_for_outlook_data_system_tall_df, colors_dict,transport_type, medium, INDEPENDENT_AXIS, SUM_OF_ALL_ECONOMIES):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        if transport_type=='passenger':
            ...
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/energy_use_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_{medium}.html')
            
            fig.update_yaxes(matches=None)
            fig.for_each_yaxis(lambda yaxis: yaxis.update(showticklabels=True))
            #write to html
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/energy_use_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_{medium}_indpax.html')
...
def emissions_by_fuel_type_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, emissions_factors, model_output_with_fuels_df, colors_dict,transport_type, INDEPENDENT_AXIS, SUM_OF_ALL_ECONOMIES, USE_AVG_GENERATION_EMISSIONS_FACTOR=True):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        if transport_type=='passenger':
            ...
            #save to html
            fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/emissions_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}.html')
            ...
            if INDEPENDENT_AXIS:
                ...
                #save to html
                fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/emissions_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_indpax.html')
            else:
...
def share_of_emissions_by_vehicle_type_FOR_MULTIPLE_ECONOMIES(economy_grouping_name, emissions_factors, model_output_with_fuels_df, colors_dict, SUM_OF_ALL_ECONOMIES, USE_AVG_GENERATION_EMISSIONS_FACTOR=True):
    ...
    for scenario in emissions_by_vehicle_type['Scenario'].unique():
        ...
        #save to html
        fig.write_html(f'plotting_output/dashboards/multiple_economy_dashboards/share_of_emissions_by_vehicle_type_{scenario}_{economy_grouping_name}.html')
    return
