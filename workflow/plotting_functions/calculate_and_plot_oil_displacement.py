...
#%%
def calculate_and_plot_oil_displacement(ECONOMY_ID, CHART_OPTION='stacked_bar', COMBINE_LPVS = True, bar_graph_year_intervals=False, INCLUDE_VTYPES = False, INCLUDE_OIL_USE=True):
        ...
        for t_type in unique_transport_types:
            for scenario in model_output_detailed.Scenario.unique():
                ...
                if CHART_OPTION == 'stacked_area_with_difference':
                    plot_stacked_area_with_difference(ice_bev, v_types, color_map, default_save_folder,t_type,  economy, scenario,  AUTO_OPEN_PLOTLY_GRAPHS, drive = 'bev')
                elif CHART_OPTION == 'stacked_bar':
                    plot_stacked_bar(ice_bev, v_types, color_map, default_save_folder, t_type, economy, scenario, AUTO_OPEN_PLOTLY_GRAPHS, drive='bev', INCLUDE_OIL_USE=INCLUDE_OIL_USE)
                ...
                ############plotting
                if CHART_OPTION == 'stacked_area_with_difference':
                    plot_stacked_area_with_difference(ice_fcev, v_types, color_map, default_save_folder,t_type,  economy, scenario,  AUTO_OPEN_PLOTLY_GRAPHS, drive = 'fcev')
                elif CHART_OPTION == 'stacked_bar':
                    plot_stacked_bar(ice_fcev, v_types, color_map, default_save_folder, t_type, economy, scenario, AUTO_OPEN_PLOTLY_GRAPHS, drive='fcev', INCLUDE_OIL_USE=INCLUDE_OIL_USE)

def plot_stacked_area_with_difference(df, v_types, color_map, default_save_folder,t_type,  economy, scenario, AUTO_OPEN_PLOTLY_GRAPHS, drive):
    if drive == 'bev':
        fig = go.Figure()
...
def plot_stacked_bar(df, v_types, color_map, default_save_folder, t_type, economy, scenario, AUTO_OPEN_PLOTLY_GRAPHS, drive, INCLUDE_OIL_USE):
    if drive == 'bev':
        ...
        fig.write_html(f'{default_save_folder}/BAR_oil_displacement_{t_type}_{economy}_bev_{scenario}.html', auto_open=AUTO_OPEN_PLOTLY_GRAPHS)
    if drive == 'fcev':
        fig = go.Figure()
        for v_type in v_types:
            fig.add_trace(go.Bar(
        ...
        fig.write_html(f'{default_save_folder}/BAR_oil_displacement_{t_type}_{economy}_fcev_{scenario}.html', auto_open=AUTO_OPEN_PLOTLY_GRAPHS)
#%%
# ECONOMY_ID = '12_NZ'
