...
def prepare_fig_dict_and_subfolders(ECONOMY_IDs, plots, ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR):
    ...
    for economy in config.economy_scenario_concordance['Economy'].unique():
        if economy in ECONOMY_IDs:
            ...
            if not ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR and not os.path.exists('plotting_output/dashboards/{}/{}'.format(economy,config.OUTLOOK_BASE_YEAR)):#put plots in a subfolder if we are projecting to the outlook base year
            ...
            elif ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR and not os.path.exists('plotting_output/dashboards/{}'.format(economy)):
...
def create_dashboard(ECONOMY_IDs, plots, DROP_NON_ROAD_TRANSPORT, colors_dict, dashboard_name_id, hidden_legend_names, ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR, ARCHIVE_PREVIOUS_DASHBOARDS, CREATE_SINGLE_TRANSPORT_TYPE_MEDIUM_PLOTS_DICT=None,PRODUCE_AS_SINGLE_POTS=False, PREVIOUS_PROJECTION_FILE_DATE_ID=None):
    ...
    for economy in fig_dict.keys():
        ...
        if PRODUCE_AS_SINGLE_POTS:
            ...
            if not os.path.exists('plotting_output/dashboards/{}/{}'.format(economy, dashboard_name_id)):
                os.makedirs('plotting_output/dashboards/{}/{}'.format(economy, dashboard_name_id))
...
def load_and_format_input_data(ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR, ECONOMY_IDs, PREVIOUS_PROJECTION_FILE_DATE_ID):
    ...
    def assign_FILE_DATE_ID(ECONOMY_IDs):
        ...
        for economy in ECONOMY_IDs:
            if os.path.exists('output_data/model_output_with_fuels/{}_{}'.format(economy, config.model_output_file_name)):
                ECONOMY_IDs_dict[economy] = config.FILE_DATE_ID
            else:
                date_id = utility_functions.get_latest_date_for_data_file('output_data/model_output_with_fuels', '{}_'.format(economy))
    ...
    for economy in ECONOMY_IDs_dict.keys():
        ...
        model_output_with_fuels_ = pd.read_csv('output_data/model_output_with_fuels/{}_{}'.format(economy, model_output_file_name))
        model_output_detailed_ = pd.read_csv('output_data/model_output_detailed/{}_{}'.format(economy, model_output_file_name))
        energy_output_for_outlook_data_system_ = pd.read_csv(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_{date_id}_transport_energy_use.csv')
        chargers_ = pd.read_csv('output_data/for_other_modellers/charging/{}_estimated_number_of_chargers.csv'.format(economy))
        supply_side_fuel_mixing_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_supply_side_fuel_mixing.csv'.format(date_id, economy))
        demand_side_fuel_mixing_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_demand_side_fuel_mixing.csv'.format(date_id, economy))
        road_model_input_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_road_model_input_wide.csv'.format(date_id, economy))
        model_output_detailed_detailed_non_road_drives_ = pd.read_csv('output_data/model_output_detailed/{}_NON_ROAD_DETAILED_{}'.format(economy, model_output_file_name))
        growth_forecasts_ = pd.read_csv(f'intermediate_data/model_inputs/{date_id}/{economy}_growth_forecasts_wide.csv')
        first_road_model_run_data_ = pd.read_csv('intermediate_data/road_model/first_run_{}_{}'.format(economy, model_output_file_name))
        new_sales_shares_all_plot_drive_shares_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_vehicle_sales_share.csv'.format(date_id, economy))
        gompertz_parameters_df_ = pd.read_csv('intermediate_data/road_model/{}_parameters_estimates_{}.csv'.format(economy, date_id)) 
        activity_change_for_plotting_ = pd.read_csv('intermediate_data/model_outputs/{}_medium_to_medium_activity_change_for_plotting{}.csv'.format(economy, date_id))
        bunkers_data_ = pd.read_csv(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_international_bunker_energy_use_{date_id}.csv'.format(economy))
        ...
        if PREVIOUS_PROJECTION_FILE_DATE_ID!=None:
            previous_projection_energy_output_for_outlook_data_system_ = pd.read_csv(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_{PREVIOUS_PROJECTION_FILE_DATE_ID}_transport_energy_use.csv')
            previous_bunkers_data_ = pd.read_csv(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_international_bunker_energy_use_{PREVIOUS_PROJECTION_FILE_DATE_ID}.csv'.format(economy))
        ##################
...
def remove_old_dashboards(ECONOMIES_TO_SKIP, dashboard_name_id):
    ...
    for economy in config.economy_scenario_concordance['Economy'].unique().tolist():
        if economy not in ECONOMIES_TO_SKIP:
            for file in os.listdir(f'plotting_output/dashboards/{economy}/'):
                if dashboard_name_id in file:
                    os.remove(f'plotting_output/dashboards/{economy}/{file}')
...
def dashboard_creation_handler(ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR, ECONOMY_ID=None, ARCHIVE_PREVIOUS_DASHBOARDS=False, PREVIOUS_PROJECTION_FILE_DATE_ID=None):
    ...
    if PREVIOUS_PROJECTION_FILE_DATE_ID != None:
        ...
        # plots.append(f'compare_energy_vs_previous_all_ESTO')
    ...
    #     plots.append(f'vehicle_type_stocks')
