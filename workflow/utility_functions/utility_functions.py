    ...
    for economy in config.ECONOMY_LIST:
        if economy == ECONOMY_ID or ECONOMY_ID == 'all':
            for scenario in config.SCENARIOS_LIST:
                ...
                output_file_paths.append(output_folder_path + '/' + economy + f'/{economy}_{scenario}_{config.FILE_DATE_ID}_dashboard_results.html')
                
                useful_file_paths.append('plotting_output/dashboards/' + economy + f'/{economy}_{scenario}_dashboard_assumptions.html')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{economy}_{scenario}_{config.FILE_DATE_ID}_dashboard_assumptions.html')
                
                useful_file_paths.append('plotting_output/dashboards/' + economy + f'/{economy}_{scenario}_dashboard_assumptions_extra.html')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{economy}_{scenario}_{config.FILE_DATE_ID}_dashboard_assumptions_extra.html')
                ...
                useful_file_paths.append(f'output_data/for_other_modellers/cost_estimation/{config.FILE_DATE_ID}_{economy}_cost_inputs.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{config.FILE_DATE_ID}_{economy}_cost_inputs.csv')
                
                useful_file_paths.append(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_{config.FILE_DATE_ID}_transport_stocks.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{config.FILE_DATE_ID}_{economy}_transport_stocks.csv')
                
                useful_file_paths.append(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_{config.FILE_DATE_ID}_transport_activity.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{config.FILE_DATE_ID}_{economy}_transport_activity.csv')
                
                useful_file_paths.append(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_{config.FILE_DATE_ID}_transport_stock_shares.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{config.FILE_DATE_ID}_{economy}_transport_stock_shares.csv')
                # output_data\for_other_modellers\charging
                
                useful_file_paths.append(f'output_data/for_other_modellers/charging/{economy}_estimated_number_of_chargers.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{economy}_{config.FILE_DATE_ID}_estimated_number_of_chargers.csv')
                
                useful_file_paths.append(f'output_data/model_output_detailed/{economy}_NON_ROAD_DETAILED_model_output{config.FILE_DATE_ID}.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{config.FILE_DATE_ID}_{economy}_detailed_incl_non_road.csv')
            
                useful_file_paths.append(f'output_data/for_other_modellers/output_for_outlook_data_system/{economy}_international_bunker_energy_use_{config.FILE_DATE_ID}.csv')
                output_file_paths.append(output_folder_path + '/' + economy + f'/{economy}_international_bunker_energy_use_{config.FILE_DATE_ID}.csv')
...
def find_latest_folder_via_regex(directory, pattern=r"^\d{4}((0[1-9])|(1[0-2]))((0[1-9])|([12]\d)|(3[01]))$"):
...
def produce_data_system_data_for_others():
    ...
    transport_data_system_extract = pd.read_csv('intermediate_data/model_inputs/transport_data_system_extract.csv')
...
def create_glossary():
    ...
    with open('plotting_output/glossary.txt', 'w') as f:
        for key, value in glossary_dict.items():
            f.write(f'{key}: {value}\n')
            
#%%
def compare_versions_of_input_spreadsheets(file_name1, file_name2, sheet1_name, sheet2_name, value_cols= ['Target', 'Reference'], key_cols=['Region', 'Medium', 'Vehicle Type', 'Drive', 'Date']):
    ...
    non_matching_values.to_csv('plotting_output/non_matching_values.csv')
...
def replicate_data_from_fuel_mixing_for_new_fuel_for_all_economys():
    ...
    replicated_data.to_csv('replicated_data.csv', index=False)
