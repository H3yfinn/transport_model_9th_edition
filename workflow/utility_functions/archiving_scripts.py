...
def create_archiving_folder_for_FILE_DATE_ID():
    ...
    if config.FILE_DATE_ID == '':
        ...
        if os.path.exists(archive_folder_name):
            ...
            if not os.path.exists('input_data/previous_run_archive/'):
...
def save_economy_projections_and_all_inputs(ECONOMY_ID,  ZIP_UP_ARCHIVE_FOLDER=True, ARCHIVED_FILE_DATE_ID=None,transport_data_system_FILE_DATE_ID_2=None):
    ...
    if transport_data_system_FILE_DATE_ID_2 is None:
        ...
        if not os.path.exists(f'intermediate_data/input_data_optimisations/optimised_data_{ECONOMY_ID}_{ARCHIVED_FILE_DATE_ID}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
            ARCHIVED_FILE_DATE_ID_2 = utility_functions.get_latest_date_for_data_file(data_folder_path='intermediate_data/input_data_optimisations', file_name_start=f'optimised_data_{ECONOMY_ID}_', file_name_end=f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    ...
    #create folder:
    archive_folder_name = 'output_data/archived_runs/{}_{}'.format(ECONOMY_ID, ARCHIVED_FILE_DATE_ID) + '_' + datetime.datetime.now().strftime("%H%M") 
    ...
    for file in files_list:
        ...
        dotdot = False
        if '/' in file:
            if file.startswith('../transport_data_system'):
                file = file[3:]
                dotdot = True
        ...
        if dotdot:
            shutil.copyfile('../' +file, archive_folder_name+'/'+file)
        else:
            shutil.copyfile(file, archive_folder_name+'/'+file)
    ...
    shutil.copyfile(f'input_data/9th_model_inputs/model_df_wide_{date_id}.csv', archive_folder_name+f'/input_data/9th_model_inputs/model_df_wide_{date_id}.csv')
    
    macro_date_id = utility_functions.get_latest_date_for_data_file('input_data/macro', 'APEC_GDP_data_')
    # macro_df_wide = pd.read_csv(f'input_data\macro\APEC_GDP_data_{macro_date_id}.csv')
    #create folder structure
    if not os.path.exists(archive_folder_name+'/input_data/macro'):
        os.makedirs(archive_folder_name+'/input_data/macro')
    shutil.copyfile(f'input_data/macro/APEC_GDP_data_{macro_date_id}.csv', archive_folder_name+f'/input_data/macro/APEC_GDP_data_{macro_date_id}.csv')
    ...
    optimisation_data_date_id =utility_functions.get_latest_date_for_data_file('intermediate_data/input_data_optimisations', f'optimised_data_{ECONOMY_ID}_', f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    if not os.path.exists(f'intermediate_data/input_data_optimisations/optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
    ...
    shutil.copyfile(f'intermediate_data/input_data_optimisations/optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl', archive_folder_name+f'/intermediate_data/input_data_optimisations/optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl')
    
    #move the bunkers data
    date_id = utility_functions.get_latest_date_for_data_file('output_data/for_other_modellers/output_for_outlook_data_system', '{}_international_bunker_energy_use_'.format(ECONOMY_ID))
    ...
    shutil.copyfile('output_data/for_other_modellers/output_for_outlook_data_system/{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id), archive_folder_name+'/output_data/for_other_modellers/output_for_outlook_data_system/{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id))
    
    #do same thing but for international_bunker_energy_use_{date_id}.csv
    shutil.copyfile('output_data/for_other_modellers/output_for_outlook_data_system/international_bunker_energy_use_{}.csv'.format(date_id), archive_folder_name+'/output_data/for_other_modellers/output_for_outlook_data_system/international_bunker_energy_use_{}.csv'.format(date_id))
    ...
    for file in os.listdir('output_data/for_other_modellers/{}'.format(ECONOMY_ID)):
        shutil.copyfile('output_data/for_other_modellers/{}/'.format(ECONOMY_ID)+file, archive_folder_name+'/output_data/for_other_modellers/{}/'.format(ECONOMY_ID)+file)
    ...
    for file in os.listdir('input_data/user_input_spreadsheets'):
        if file.endswith('.csv'):
            shutil.copyfile('input_data/user_input_spreadsheets/'+file, archive_folder_name+'/input_data/user_input_spreadsheets/'+file)
    ...
    if os.path.exists('intermediate_data/road_model/change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID)):
        ...
        shutil.copyfile('intermediate_data/road_model/change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID), archive_folder_name+ '/intermediate_data/road_model/change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID))
    ...
    for file in os.listdir(f'plotting_output/dashboards/{ECONOMY_ID}'):
        if file.endswith('.html'):
            shutil.copyfile(f'plotting_output/dashboards/{ECONOMY_ID}/'+file, archive_folder_name+f'/plotting_output/dashboards/{ECONOMY_ID}/'+file)              
                
    #save entire workflow folder
    recursively_save_file('./workflow', archive_folder_name+'/workflow', file_extension='.py', exclude_archive_folder=True, keep_folder_structure=True) 
    ...
    for file in os.listdir('config/concordances_and_config_data'):
        if file.endswith('.csv'):
            shutil.copyfile('config/concordances_and_config_data/'+file, archive_folder_name+'/config/concordances_and_config_data/'+file)
    ...
    # # input_data_based_on_previous_model_run.to_csv('intermediate_data/model_outputs/{}_input_data_based_on_previous_model_run_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    # # energy_use_output.to_csv('intermediate_data/model_outputs/{}_energy_use_output_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    #     demand_side_fuel_mixing = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))        
    ...
    # #     vehicles_per_stock_parameters.to_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters.csv'.format(ECONOMY_ID), index=False)
    #find latest version of vehicles_per_stock_parameters
    # #save parameters_estimates. at the very elast we will plot these later
    # parameters_estimates.to_csv('intermediate_data/road_model/{}_parameters_estimates_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID), index=False)
    ...
    # new_sales_shares_all_new.to_csv('intermediate_data/model_inputs/{}/{}_vehicle_sales_share.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index = False)
    ...
    #     model_output_with_fuels_ = pd.read_csv('output_data/model_output_with_fuels/{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     model_output_detailed_ = pd.read_csv('output_data/model_output_detailed/{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     energy_output_for_outlook_data_system_ = pd.read_csv(f'output_data/for_other_modellers/output_for_outlook_data_system/{ECONOMY_ID}_{config.FILE_DATE_ID}_transport_energy_use.csv')
    #     chargers_ = pd.read_csv('output_data/for_other_modellers/charging/{}_estimated_number_of_chargers.csv'format(ECONOMY_ID))
    #     supply_side_fuel_mixing_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     road_model_input_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     model_output_detailed_detailed_non_road_drives_ = pd.read_csv('output_data/model_output_detailed/{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     growth_forecasts_ = pd.read_csv(f'intermediate_data/model_inputs/{config.FILE_DATE_ID}/{ECONOMY_ID}_growth_forecasts_wide.csv')
    #     first_road_model_run_data_ = pd.read_csv('intermediate_data/road_model/first_run_{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     new_sales_shares_all_plot_drive_shares_ = pd.read_csv('intermediate_data/model_inputs/{}/{}_vehicle_sales_share.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     gompertz_parameters_df_ = pd.read_csv('intermediate_data/road_model/{}_parameters_estimates_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID))
    ...
    # supply_side_fuel_mixing = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID,ECONOMY_ID))
    # demand_side_fuel_mixing = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # road_model_input_wide = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # non_road_model_input_wide = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_non_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # growth_forecasts_wide = pd.read_csv('intermediate_data/model_inputs/{}/{}_aggregated_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # macro2.to_csv('intermediate_data/model_inputs/regression_based_growth_estimates.csv', index=False)
...
def revert_to_previous_version_of_files(economy, archive_folder_name, UNZIP_ARCHIVE_FOLDER=True, CURRENT_FILE_DATE_ID=None):
    ...
    #now using the previous files, save them all back where they came from. However, dont overwrite the files in workflow/utility_functions/* since that includes this function!
    ...
    #recursively go through all files in the archive_folder_name and save them back to where they came from (except for the files in workflow/utility_functions/* and transport_data_system/*, which will be saved in the transport_data_system folder)
    breakpoint()
    for root, dirs, files in os.walk(archive_folder_name):
        for file in files:
            if file.endswith('.py') or file.endswith('.yaml') or file.endswith('.yaml') or file.endswith('.csv') or file.endswith('.xlsx') or file.endswith('.pkl') or file.endswith('.html'):
                ...
                root = root.replace(archive_folder_name, '')
                root = root.strip('\\')
                
                if root.startswith('workflow/utility_functions'):
                    continue
                if root.startswith('transport_data_system'):
                    #check folder exists
