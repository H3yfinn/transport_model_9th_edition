...
def archive_lots_of_files(archive_folder_name):
    ...
    shutil.copyfile(root_dir + '/' + 'config/config.py', '{}/config.py'.format(archive_folder_name))

    #save all workflow files to folder, incmluding all subfolders (they are the code).
    recursively_save_file(root_dir + '/' + './workflow', archive_folder_name, 'file_extension=.py', exclude_archive_folder=True)
...
def save_economy_projections_and_all_inputs(ECONOMY_ID,  ZIP_UP_ARCHIVE_FOLDER=True, ARCHIVED_FILE_DATE_ID=None,transport_data_system_FILE_DATE_ID_2=None):
    ...
    recursively_save_file(root_dir + '/' + './workflow', archive_folder_name+'/workflow', file_extension='.py', exclude_archive_folder=True, keep_folder_structure=True) 
