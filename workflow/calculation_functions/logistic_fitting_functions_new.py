...
def calculate_vehicles_per_stock_parameters(model_data,ECONOMY_ID, car_base_amount=1,lcv_base_amount=3):
    ...
    if vehicles_per_stock_parameters.isna().sum().sum()>0:
        ...
        for economy in config.economy_scenario_concordance['Economy'].unique():
            # if os.path.exists('intermediate_data/road_model/{}_vehicles_per_stock_parameters_{}.csv'.format(economy, 'passenger_only')) & ONLY_PASSENGER_VEHICLES:
            #     e = pd.read_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters_{}.csv'.format(economy, 'passenger_only'))
            if os.path.exists('intermediate_data/road_model/{}_vehicles_per_stock_parameters.csv'.format(economy)):
                e = pd.read_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters.csv'.format(economy))
            else:
                continue
    ...
    #     vehicles_per_stock_parameters.to_csv('intermediate_data/road_model/{}_vehicles_per_stock_parameters_{}_{}.csv'.format(ECONOMY_ID, 'passenger_only'), index=False)
    # else:
...
def prepare_data_for_logistic_fitting(model_data, ECONOMY_ID):
    ...
    First step is to calculate_vehicles_per_stock_parameters(). These are used to adjust certain vehicle types to represent the relative amount of activity they do. e.g. factor for 2w is <1 and factor for ht >1 because 2w dont carry as much, or drive as much as ht. This is done especailly so countries with lots of 2w dont have a higher stocks per capita than countries with lots of cars.
...
def create_new_dataframe_with_logistic_predictions(new_model_data, new_stocks_per_capita_estimates, FIT_LOGISTIC_CURVE_TO_DATA):
    ...
    # model_data_logistic_predictions.to_csv('b.csv')
