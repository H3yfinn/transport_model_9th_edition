...
def create_archiving_folder_for_FILE_DATE_ID(config):
    ...
    if config.FILE_DATE_ID == '':
        archive_folder_name = config.root_dir + '\\' + 'input_data\\previous_run_archive\\latest_test_run'
    else:
        archive_folder_name = config.root_dir + '\\' + 'input_data\\previous_run_archive\\' + config.FILE_DATE_ID
        ...
        if os.path.exists(archive_folder_name):
            ...
            archive_folder_name = config.root_dir + '\\' + 'input_data\\previous_run_archive\\' + config.FILE_DATE_ID +'\\'+ new_FILE_DATE_ID
        
            if not os.path.exists(archive_folder_name):
                os.mkdir(archive_folder_name)
        else:
            if not os.path.exists(config.root_dir + '\\' + 'input_data\\previous_run_archive\\'):
                os.mkdir(config.root_dir + '\\' + 'input_data\\previous_run_archive\\')
            os.mkdir(archive_folder_name)
...
def archive_lots_of_files(config, archive_folder_name):
    ...
    model_output_non_detailed = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output\\{}'.format(config.model_output_file_name))
    model_output_all_with_fuels = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output_with_fuels\\{}'.format(config.model_output_file_name))

    #save output data
    model_output_detailed.to_csv(config.root_dir + '\\' + '{}\\model_output_detailed.csv'.format(archive_folder_name))
    model_output_non_detailed.to_csv(config.root_dir + '\\' + '{}\\model_output_non_detailed.csv'.format(archive_folder_name))
    model_output_all_with_fuels.to_csv(config.root_dir + '\\' + '{}\\model_output_all_with_fuels.csv'.format(archive_folder_name))
    
    #save config file to folder
    shutil.copyfile(config.root_dir + '\\' + 'code\\config.py', '{}\\config.py'.format(archive_folder_name))

    #save all code files to folder, incmluding all subfolders (they are the code).
    recursively_save_file(config.root_dir + '\\' + 'code', archive_folder_name, 'file_extension=.py', exclude_archive_folder=True)
    #save all csvs in \input_data\user_input_spreadsheets
    recursively_save_file(config.root_dir + '\\' + 'input_data\\user_input_spreadsheets', archive_folder_name, file_extension='.csv', exclude_archive_folder=True)
    recursively_save_file(config.root_dir + '\\' + f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}', archive_folder_name, file_extension='.csv', exclude_archive_folder=True)
    recursively_save_file(config.root_dir + '\\' + 'output_data\\for_other_modellers', archive_folder_name, exclude_archive_folder=True)
    
    #and save individual files
    fuel_mixing_assumptions = config.root_dir + '\\' + 'input_data\\fuel_mixing_assumptions.xlsx'
    shutil.copyfile(fuel_mixing_assumptions, '{}\\fuel_mixing_assumptions.xlsx'.format(archive_folder_name))

    recursively_save_file(config.root_dir + '\\' + 'config\\concordances_and_config_data\\computer_generated_concordances', archive_folder_name, '.csv', exclude_archive_folder=True)
...
def zip_up_folder(config, archive_folder_name):
    # if os.path.exists(config.root_dir + '\\' + 'C:\\Users\\finbar.maunsell\\Documents'):
...
def save_economy_projections_and_all_inputs(config, ECONOMY_ID, ZIP_UP_ARCHIVE_FOLDER=True, ARCHIVED_FILE_DATE_ID=None, transport_data_system_FILE_DATE_ID_2=None):
    ...
    if transport_data_system_FILE_DATE_ID_2 is None:
        ...
        if not os.path.exists(config.root_dir + '\\' + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{ARCHIVED_FILE_DATE_ID}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
            ARCHIVED_FILE_DATE_ID_2 = utility_functions.get_latest_date_for_data_file(data_folder_path=config.root_dir + '\\' +'intermediate_data\\input_data_optimisations', file_name_start=f'optimised_data_{ECONOMY_ID}_', file_name_end=f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    ...
    #create folder:
    archive_folder_name = config.root_dir + '\\' + 'output_data\\archived_runs\\{}_{}'.format(ECONOMY_ID, ARCHIVED_FILE_DATE_ID) + '_' + datetime.datetime.now().strftime("%H%M") 
    ...
    for file in sometimes_files:
        if os.path.exists(config.root_dir + '\\' + file):
            files_list.append(config.root_dir + '\\' + file)
    ...
    for file in files_list:
        ...
        dotdot = False
        if '\\' in file:
            if file.startswith(config.root_dir + '\\' + '..\\transport_data_system'):
                file = file[3:]
                dotdot = True
        ...
        if dotdot:
            shutil.copyfile(config.root_dir + '\\' + '..\\' +file, archive_folder_name+'\\'+file)
        else:
            shutil.copyfile(config.root_dir + '\\' + file, archive_folder_name+'\\'+file)
    ...
    shutil.copyfile(config.root_dir + '\\' + f'input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv', archive_folder_name+f'\\input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv')
    
    macro_date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + '\\' + 'input_data\\macro', 'APEC_GDP_data_')
    # macro_df_wide = pd.read_csv(config.root_dir + '\\' +f'input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv')
    #create folder structure
    if not os.path.exists(archive_folder_name+'\\input_data\\macro'):
        os.makedirs(archive_folder_name+'\\input_data\\macro')
    shutil.copyfile(config.root_dir + '\\' + f'input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv', archive_folder_name+f'\\input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv')
    ...
    optimisation_data_date_id =utility_functions.get_latest_date_for_data_file(config.root_dir + '\\' + 'intermediate_data\\input_data_optimisations', f'optimised_data_{ECONOMY_ID}_', f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    if not os.path.exists(config.root_dir + '\\' + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
    ...
    shutil.copyfile(config.root_dir + '\\' + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl', archive_folder_name+f'\\intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl')
    
    #move the bunkers data
    date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + '\\' + 'output_data\\for_other_modellers\\output_for_outlook_data_system', '{}_international_bunker_energy_use_'.format(ECONOMY_ID))
    ...
    shutil.copyfile(config.root_dir + '\\' + 'output_data\\for_other_modellers\\output_for_outlook_data_system\\{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id), archive_folder_name+'\\output_data\\for_other_modellers\\output_for_outlook_data_system\\{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id))
    
    #do same thing but for international_bunker_energy_use_{date_id}.csv
    shutil.copyfile(config.root_dir + '\\' + 'output_data\\for_other_modellers\\output_for_outlook_data_system\\international_bunker_energy_use_{}.csv'.format(date_id), archive_folder_name+'\\output_data\\for_other_modellers\\output_for_outlook_data_system\\international_bunker_energy_use_{}.csv'.format(date_id))
    ...
    if os.path.exists(config.root_dir + '\\' + 'intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID)):
        ...
        shutil.copyfile(config.root_dir + '\\' + 'intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID), archive_folder_name+ '\\intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID))
    ...
    for file in os.listdir(config.root_dir + '\\' + f'plotting_output\\dashboards\\{ECONOMY_ID}'):
        if file.endswith('.html'):
            shutil.copyfile(config.root_dir + '\\' + f'plotting_output\\dashboards\\{ECONOMY_ID}\\'+file, archive_folder_name+f'\\plotting_output\\dashboards\\{ECONOMY_ID}\\'+file)              
                
    #save entire code folder
    recursively_save_file(config.root_dir + '\\' + 'code', archive_folder_name+'\\code', file_extension='.py', exclude_archive_folder=True, keep_folder_structure=True) 
    ...
    # # input_data_based_on_previous_model_run.to_csv(config.root_dir + '\\' + 'intermediate_data\\model_outputs\\{}_input_data_based_on_previous_model_run_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    # # energy_use_output.to_csv(config.root_dir + '\\' + 'intermediate_data\\model_outputs\\{}_energy_use_output_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    # road_model_input = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\{}\\{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # growth_forecasts = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\{}\\{}_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    ...
    # model_output_detailed.to_csv(config.root_dir + '\\' + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False
    # model_output_non_detailed.to_csv(config.root_dir + '\\' + 'output_data\\model_output\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False
    # model_output_all_with_fuels.to_csv(config.root_dir + '\\' + 'output_data\\model_output_with_fuels\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_detailed.to_csv(config.root_dir + '\\' + 'output_data\\model_output_detailed\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_non_detailed.to_csv(config.root_dir + '\\' + 'output_data\\model_output\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_all_with_fuels.to_csv(config.root_dir + '\\' + 'output_data\\model_output_with_fuels\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # road_model_output = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\road_model\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    # model_output_all_with_fuels = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output_with_fuels\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    #     model_output_with_fuels_ = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output_with_fuels\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     model_output_detailed_ = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output_detailed\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     energy_output_for_outlook_data_system_ = pd.read_csv(config.root_dir + '\\' +f'output_data\\for_other_modellers\\output_for_outlook_data_system\\{ECONOMY_ID}_{config.FILE_DATE_ID}_transport_energy_use.csv')
    #     chargers_ = pd.read_csv(config.root_dir + '\\' + 'output_data\\for_other_modellers\\charging\\{}_estimated_number_of_chargers.csv'format(ECONOMY_ID))
    #     supply_side_fuel_mixing_ = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\{}\\{}_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     road_model_input_ = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\{}\\{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     model_output_detailed_detailed_non_road_drives_ = pd.read_csv(config.root_dir + '\\' + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     growth_forecasts_ = pd.read_csv(config.root_dir + '\\' +f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}\\{ECONOMY_ID}_growth_forecasts_wide.csv')
    #     first_road_model_run_data_ = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\road_model\\first_run_{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     new_sales_shares_all_plot_drive_shares_ = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\{}\\{}_vehicle_sales_share.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     gompertz_parameters_df_ = pd.read_csv(config.root_dir + '\\' + 'intermediate_data\\road_model\\{}_parameters_estimates_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID))
    ...
    # macro2.to_csv(config.root_dir + '\\' + 'intermediate_data\\model_inputs\\regression_based_growth_estimates.csv', index=False)
    # transport_data_system_folder = '..\\transport_data_system'
    # transport_data_system_df = pd.read_csv(config.root_dir + '\\' + '{}\\output_data\\combined_data_{}.csv'.format(transport_data_system_folder,config.transport_data_system_FILE_DATE_ID))
...
def revert_to_previous_version_of_files(config, economy, archive_folder_name, UNZIP_ARCHIVE_FOLDER=True, CURRENT_FILE_DATE_ID=None):
    ...
    breakpoint()#NOTE AFTER CREATING config.root_dir + '\\' + FUNCTIONALITY, I DONT KNOW IF THIS IS WORKING SO IF SET IT TO RAISE AN ERROR
    ...
    for root, dirs, files in os.walk(archive_folder_name):
        for file in files:
            if file.endswith('.py') or file.endswith('.yaml') or file.endswith('.yaml') or file.endswith('.csv') or file.endswith('.xlsx') or file.endswith('.pkl') or file.endswith('.html'):
                ...
                root = root.replace(archive_folder_name, config.root_dir + '\\' + '')
                root = root.strip('\\')
                
                if root.startswith('code\\utility_functions'):
                    continue
                if root.startswith(config.root_dir + '\\' +'transport_data_system'):
                    #check folder exists
                    if not os.path.exists(config.root_dir + '\\' + '..\\'+root):
                        os.makedirs('..\\'+root)
