...
def prepare_comparison_inputs(config, growth_coefficients_df, energy_macro, BASE_YEAR_activity_9th, activity_growth_8th, activity_8th, independent_variables, models):
    ...
    useful_macro_vars = ['Population', 'Gdp', 'Gdp_per_capita', 'Gdp_times_capita', 'Gdp_growth', 'Population_growth', 'Gdp_per_capita_growth', 'Gdp_times_capita_growth']
    #if any if those vars arent in energy_macro then drop them
    for useful_macro_var in useful_macro_vars:
        if useful_macro_var not in energy_macro.columns:
            useful_macro_vars.remove(useful_macro_var)
...
def plot_and_compare_new_growth_coefficients(config, GROWTH_MEASURES_TO_PLOT, ACTIVITY_MEASURES_TO_PLOT):
    #LOAD DATA:
    measures_to_plot_path = os.path.join(config.root_dir, 'intermediate_data', 'growth_analysis', 'measures_to_plot.txt')
    indexed_measures_to_plot_path = os.path.join(config.root_dir, 'intermediate_data', 'growth_analysis', 'indexed_measures_to_plot.txt')
    df_path = os.path.join(config.root_dir, 'intermediate_data', 'growth_analysis', 'df_growth_parameter_analysis.pkl')
    
    with open(measures_to_plot_path, 'r') as f:
        measures_to_plot = [line.rstrip() for line in f]
        
    with open(indexed_measures_to_plot_path, 'r') as f:
    ...
    for economy in df['Economy'].unique():
        ...
        #write to html
        fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'growth_analysis', f'{title}.html'))
...
def plot_growth_coefficients(config, growth_coefficients_df, independent_variables):
    ...
    for model in growth_coefficients_df.Model.unique():
        ...
        #save
        fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'growth_analysis', f'{model}_growth_coefficients.html'))
...
def group_by_region(config, energy_macro, region_column = 'Region_growth_regression2'):
    ...
    regional_mapping = pd.read_csv(os.path.join(config.root_dir, 'config', 'concordances_and_config_data', 'region_economy_mapping.csv'))
