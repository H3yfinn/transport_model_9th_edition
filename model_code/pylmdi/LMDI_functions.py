...
# in all of the following functions we use the term energy but it can also be emissions, as that capability has been built into the code.
...
def Mult(config, driver_input_data, energy_data, drivers_list, structure_variables_list, energy_variable, time_variable, extra_identifier):
    ...
    # CALCULATE:
    # calculate ln(d_t/d_0) for the drivers. We will call this the 'log ratio driver value'
    ...
    # calculate the exponential of each new driver value
    for driver in drivers_list:
        driver_df[driver] = np.exp(driver_df[driver])

    # calculate ratio of energy for each year compared to the base year using the 'Change in energy_top' variable. To do this we need to rejoin the base year data to the driver df to calculate ('change in energy'/base year energy) + 1 (this is equivalent to the ratio between other years and base years in total energy)
...
def log_mean_func(config, energy_df, energy_variable):
    ## Find the weighting for the divisia function using the log mean function on the energy data. This will calculate the log mean when comparing the base year to the other years
...
def hierarchical_LMDI(config, energy_data, activity_data, energy_variable, activity_variable, structure_variables_list, time_variable, extra_identifier):
    ...
    # The process will be centred around one big_df which we will dip in and out of for variables. We will also have a list of the different structural drivers which we will loop through since there are a lot of different combinations of sectoral calculations that need to be done.

    # we will also use a numbering system in case it is useful to reference where some variables were calculated or something.

    # 1:
    # In this section we calculate and sum for:
    ...
    # Intersectoral intensity: Eij/Ai, Eijk/Aij : where E is energy and A is activity. This is what I've called the intersectoral structural intensity. We will calculate a log mean from this in 3, which will also be used to calculate weighting_value_ in 3 too.
    ...
    ########################################
    # 2
    # ln(At/A0) : which is used in calculating the activity driver
    ...
    for structure_variable in structure_variables_list[1:]:
        ...
        # remove any duplicates in the df since we are reducing the number of categorical variables
    ...
    ########################################
    # 3.
    # CALCULATE WEIGHTING VALUES FOR EACH STRUCTURE VARIABLE EXCEPT THE FIRST ONE
    # this will work assuming that the structural variables will increase as the structural variable for the driver does too.
    # so say, for the intensity and structural share drivers of drive, the formula is:
    # L(EijT/AiT, Eij0/Ai0)
    # L(EiT/AiT, Ei0/Ai0)
    # L(EijT/AiT, Eij0/Ai0) / L(EiT/AiT, Ei0/Ai0) : which is the division of the two above
    ...
    # So since we will skip the first structural variable, we will calculate this weighting value using the intersectoral intensity for the second and subsequent levels of hierarchy's structural driver.
    # the formula for this value for the second level of hierarchy would be: L(EijT/AiT, Eij0/Ai0) / L(EiT/AiT, Ei0/Ai0) where T is the time variable after the base year, 0 is the base year. i is Vehicle type, j is Drive.
    ...
    for structure_variable in structure_variables_list[1:]:
        hierarchy_list.append(structure_variable)
        intersectoral_intensity_df = big_df[['log_mean_' + structure_variable, 'intensity_log_mean_bot_' + structure_variable, time_variable] + hierarchy_list]
        intersectoral_intensity_df = intersectoral_intensity_df.drop_duplicates()
        intersectoral_intensity_df['weighting_value_' + structure_variable] = intersectoral_intensity_df['log_mean_' + structure_variable] / intersectoral_intensity_df['intensity_log_mean_bot_' + structure_variable]

        # drop the columns we don't need
        intersectoral_intensity_df.drop(columns=['log_mean_' + structure_variable, 'intensity_log_mean_bot_' + structure_variable], inplace=True)
    ...
    # BOT
    # grab Energy by year summed for the first structural category and energy summed with no structural categories
    ...
    ########
    # merge the top and bot values and calculate the weighting value
    ...
    # 8
    # please note that in the final version of this method we actually remove the intensity drivers for all structural levels except the last. But we have to calculate the intensity_log_mean_ratio for each structural level anyway so to keep the code simple we just calculate the intensity drivers for all structural levels and remove them afterwards
    ...
    # FORMATTING
    # GREAT GOOD WORK!
    # we have now calculated all the drivers. We will rename them and put them in one dataframe with a time variable and the ratio between other years and base years in total energy, then we are done!
    ...
    # for ease of use we didn't remove the intensity drivers for all structural variables except the last one. Now we will remove them.
    ...
    if abs(total_difference) > 0.01:
        breakpoint() # this started happening in 25th September. don't know why
...
def hierarchical_LMDI_emissions(config, energy_data, emissions_data, activity_data, energy_variable, emissions_variable, activity_variable, structure_variables_list, time_variable, extra_identifier):
    ...
    # PLEASE NOTE THAT THE BELOW FUNCTION IS EXPERIMENTAL AND HAS NOT BEEN PROVEN. BUT IT SEEMS TO DO WHAT IS EXPECTED.
    # save all the inputs and load them in again so we can easily test this function
    
    # This is the function for lmdi hierarchical multiple structural effects.

    # There is only one intensity effect, which is for the final structural driver (the lowest on the hierarchy), but to keep things simple we will calculate the intensity effect for all structural variables and remove the ones we don't need at the end.
    # The structural driver for each subsequent structural category down the hierarchy is calculated. The exact formula is complicated. I have put it as a screenshot in the documentation folder.

    # The code for this function is quite long, this is because it is a complicated formula and since I have tried to make it as clear as possible, it is long.

    # The process will be centered around one big_df which we will dip in and out of for variables. We will also have a list of the different structural drivers which we will loop through since there are a lot of different combinations of sectoral calculations that need to be done.
    ...
    # so treating the order of structure_variables_list as the hierarchical order:
    # calc total activity and energy for each year
    base_year = emissions_data[time_variable].min()
    # check base year is the same in both dataframes
    assert base_year == activity_data[time_variable].min()

    big_df = pd.merge(activity_data, energy_data, on=[time_variable] + structure_variables_list)
    big_df = pd.merge(activity_data, emissions_data, on=[time_variable] + structure_variables_list)
    
    big_df = pd.merge(big_df, activity_data.groupby([time_variable])[activity_variable].sum(), on=[time_variable], suffixes=('', '_total'))
    big_df = pd.merge(big_df, energy_data.groupby([time_variable])[energy_variable].sum(), on=[time_variable], suffixes=('', '_total'))
    big_df = pd.merge(big_df, emissions_data.groupby([time_variable])[emissions_variable].sum(), on=[time_variable], suffixes=('', '_total'))
    ...
    for structure_variable in structure_variables_list:
        ...
        energy_sum = energy_data.groupby([time_variable] + hierarchy_list)[energy_variable].sum().reset_index()
        emissions_sum = emissions_data.groupby([time_variable] + hierarchy_list)[emissions_variable].sum().reset_index()
        activity_sum = activity_data.groupby([time_variable] + hierarchy_list)[activity_variable].sum().reset_index()
        
        prev_activity_sum = activity_data.groupby([time_variable] + hierarchy_list[:-1])[activity_variable].sum().reset_index()
        prev_energy_sum = energy_data.groupby([time_variable] + hierarchy_list[:-1])[energy_variable].sum().reset_index()

        energy_intensity = pd.merge(energy_sum, activity_sum, on=[time_variable] + hierarchy_list)
        energy_intensity = pd.merge(energy_intensity, prev_activity_sum, on=[time_variable] + hierarchy_list[:-1], suffixes=('', '_prev'))
        energy_intensity['energy_intensity_' + structure_variable] = energy_intensity[energy_variable] / energy_intensity[activity_variable]
        
        emissions_intensity = pd.merge(emissions_sum, energy_sum, on=[time_variable] + hierarchy_list)
        emissions_intensity = pd.merge(emissions_intensity, prev_energy_sum, on=[time_variable] + hierarchy_list[:-1], suffixes=('', '_prev'))
        emissions_intensity['emissions_intensity_' + structure_variable] = emissions_intensity[emissions_variable] / emissions_intensity[energy_variable]

        if len(hierarchy_list) > 1:
            # intersectoral intensity is only calculated for the second and subsequent levels of hierarchy's structural driver
            # So for example, if we have ['Vehicle Type', 'Drive'], then we have calculated this value for Drive, but not for Vehicle Type.
            energy_intensity['intersectoral_energy_intensity_' + structure_variable] = energy_intensity[energy_variable] / energy_intensity[activity_variable + '_prev']
            energy_intensity['intersectoral_energy_intensity_' + structure_variable] = energy_intensity['intersectoral_energy_intensity_' + structure_variable].fillna(0)
            
            emissions_intensity['intersectoral_emissions_intensity_' + structure_variable] = emissions_intensity[emissions_variable] / emissions_intensity[energy_variable + '_prev']
            emissions_intensity['intersectoral_emissions_intensity_' + structure_variable] = emissions_intensity['intersectoral_emissions_intensity_' + structure_variable].fillna(0)
        
        emissions_intensity = emissions_intensity.drop(columns=[energy_variable, emissions_variable, energy_variable + '_prev']) # note that its important we still keep regular intensity values since they are useful later.
        energy_intensity = energy_intensity.drop(columns=[activity_variable, energy_variable, activity_variable + '_prev']) # note that its important we still keep regular intensity values since they are useful later.
        
        big_df = pd.merge(big_df, emissions_intensity, on=[time_variable] + hierarchy_list, how='left')
        # TODO NOT SURE IF WE SHOULD BE INCLUDING ENERGY INTENSITY HERE
        ##############

        # while here we can calculate ln(intensity_other_years/intensity_base_year) for each structure variable FOR EMISSIONS AND ENERGY
        # first create a df which only has the intensity, time variable and hierarchy list
        emissions_intensity = emissions_intensity[[time_variable, 'emissions_intensity_' + structure_variable] + hierarchy_list]
        # separate the base year and other years
        emissions_intensity_base_year = emissions_intensity[emissions_intensity[time_variable] == base_year].drop(columns=[time_variable])
        emissions_intensity_other_years = emissions_intensity[emissions_intensity[time_variable] != base_year]
        # merge the two together
        emissions_intensity_log = pd.merge(emissions_intensity_other_years, emissions_intensity_base_year, on=hierarchy_list, suffixes=('', '_base_year'))
        
        energy_intensity = energy_intensity[[time_variable, 'energy_intensity_' + structure_variable] + hierarchy_list]
        # separate the base year and other years
        energy_intensity_base_year = energy_intensity[energy_intensity[time_variable] == base_year].drop(columns=[time_variable])
        energy_intensity_other_years = energy_intensity[energy_intensity[time_variable] != base_year]
        # merge the two together
        energy_intensity_log = pd.merge(energy_intensity_other_years, energy_intensity_base_year, on=hierarchy_list, suffixes=('', '_base_year'))
                
        # calc the log
        energy_intensity_log['log_energy_intensity_ratio_{}'.format(structure_variable)] = np.log(energy_intensity_log['energy_intensity_{}'.format(structure_variable)] / energy_intensity_log['energy_intensity_{}_base_year'.format(structure_variable)])
        emissions_intensity_log['log_emissions_intensity_ratio_{}'.format(structure_variable)] = np.log(emissions_intensity_log['emissions_intensity_{}'.format(structure_variable)] / emissions_intensity_log['emissions_intensity_{}_base_year'.format(structure_variable)])
        # drop cols
        emissions_intensity_log = emissions_intensity_log.drop(columns=['emissions_intensity_{}'.format(structure_variable), 'emissions_intensity_{}_base_year'.format(structure_variable)])
        emissions_intensity = emissions_intensity[[time_variable, 'emissions_intensity_' + structure_variable] + hierarchy_list]
        
        energy_intensity_log = energy_intensity_log.drop(columns=['energy_intensity_{}'.format(structure_variable), 'energy_intensity_{}_base_year'.format(structure_variable)])
        energy_intensity = energy_intensity[[time_variable, 'energy_intensity_' + structure_variable] + hierarchy_list]
        
        big_df = pd.merge(big_df, energy_intensity_log, on=[time_variable] + hierarchy_list, how='left')
        big_df = pd.merge(big_df, emissions_intensity_log, on=[time_variable] + hierarchy_list, how='left')
        ##############
    ...
    for structure_variable in structure_variables_list[1:]:
        ...
        calc_df = big_df_2[[time_variable, 'intersectoral_emissions_intensity_' + structure_variable] + hierarchy_list]
        ...
        # keep the columns we need
        calc_df = calc_df[[time_variable] + hierarchy_list + ['emissions_intensity_log_mean_top_' + structure_variable]]
    ...
    for structure_variable in structure_variables_list[1:]:
        ...
        calc_df = big_df[[time_variable, 'intersectoral_emissions_intensity_' + structure_variable] + hierarchy_list]
    ...
    for structure_variable in structure_variables_list[1:]:
        previous_structure_variable = hierarchy_list[-1]
        intensity = big_df[['emissions_intensity_' + previous_structure_variable, time_variable] + hierarchy_list]
        intensity = intensity.drop_duplicates()
        # separate the base year and other year data and make the base year data a col in the other year data
        base_year_df = intensity[intensity[time_variable] == base_year].drop(columns=[time_variable])
        other_years_df = intensity[intensity[time_variable] != base_year]
        ...
        # keep only the columns we do need
        intensity_df_bot = intensity_df_bot[[time_variable, 'emissions_intensity_log_mean_bot_{}'.format(structure_variable)] + hierarchy_list]
    ...
    for structure_variable in structure_variables_list[1:]:
        hierarchy_list.append(structure_variable)
        intersectoral_intensity_df = big_df[['log_mean_' + structure_variable, 'emissions_intensity_log_mean_bot_' + structure_variable, time_variable] + hierarchy_list]
        intersectoral_intensity_df = intersectoral_intensity_df.drop_duplicates()
        intersectoral_intensity_df['weighting_value_' + structure_variable] = intersectoral_intensity_df['log_mean_' + structure_variable] / intersectoral_intensity_df['emissions_intensity_log_mean_bot_' + structure_variable]

        # drop the columns we don't need
        intersectoral_intensity_df.drop(columns=['log_mean_' + structure_variable, 'emissions_intensity_log_mean_bot_' + structure_variable], inplace=True)
    ...
    # grab sum for first structural variable:
    first_structure_variable = structure_variables_list[0]
    emissions_sum = emissions_data.groupby([time_variable, first_structure_variable])[emissions_variable].sum().reset_index()

    # separate the base year and other year data and make the base year data a col in the other year data
    base_year_df = emissions_sum[emissions_sum[time_variable] == base_year].drop(columns=[time_variable])
    other_years_df = emissions_sum[emissions_sum[time_variable] != base_year]
    # merge the data
    weighting_df_top = pd.merge(other_years_df, base_year_df, on=[first_structure_variable], suffixes=('', '_base_year'))
    # calc log mean
    ...
    # BOT
    # grab Energy by year summed for the first structural category and energy summed with no structural categories
    total_emissions_sum = big_df[[emissions_variable + '_total', time_variable]]
    # remove duplicates
    total_emissions_sum = total_emissions_sum.drop_duplicates()
    # separate the base year and other year data and make the base year data a col in the other year data
    base_year_df = total_emissions_sum[total_emissions_sum[time_variable] == base_year]
    other_years_df = total_emissions_sum[total_emissions_sum[time_variable] != base_year]
    # create a df with the base year data as a column in the other year data
    weighting_df_bot = other_years_df.copy()
    ...
    for structure_variable in structure_variables_list[1:]:
        ...
        # get the data for this level
        successive_structural_level_drivers = big_df[[time_variable, 'log_structural_share_{}'.format(structure_variable), 'log_emissions_intensity_ratio_{}'.format(structure_variable), 'log_energy_intensity_ratio_{}'.format(structure_variable), 'weighting_value_{}'.format(structure_variable), structure_variable] + hierarchy_list]
        ...
        # times the weighting_value_ by the log value for the current level (current level = structure_variable)
        successive_structural_level_drivers['emissions_intensity_driver_{}'.format(str(structure_variable_index))] = successive_structural_level_drivers['log_emissions_intensity_ratio_{}'.format(structure_variable)] * successive_structural_level_drivers['weighting_value_{}'.format(structure_variable)]
        
        successive_structural_level_drivers['energy_intensity_driver_{}'.format(str(structure_variable_index))] = successive_structural_level_drivers['log_energy_intensity_ratio_{}'.format(structure_variable)] * successive_structural_level_drivers['weighting_value_{}'.format(structure_variable)]

        successive_structural_level_drivers['structural_driver_{}'.format(str(structure_variable_index))] = successive_structural_level_drivers['log_structural_share_{}'.format(structure_variable)] * successive_structural_level_drivers['weighting_value_{}'.format(structure_variable)]


        # sum the successive_structural_level_drivers by year and all the structural levels except the current one
        successive_structural_level_drivers = successive_structural_level_drivers[[time_variable, 'emissions_intensity_driver_{}'.format(str(structure_variable_index)), 'energy_intensity_driver_{}'.format(str(structure_variable_index)), 'structural_driver_{}'.format(str(structure_variable_index))] + hierarchy_list].groupby([time_variable] + hierarchy_list).sum(numeric_only=True).reset_index()
        ...
        for hierarchy_variable in hierarchy_list[::-1]:
            if hierarchy_variable != hierarchy_list[0]: # ignore the first one (i) because the calc for that is different.
                ...
                # times the weighting_value_ by the successive_structural_level_drivers_
                successive_structural_level_drivers['emissions_intensity_driver_{}'.format(str(structure_variable_index))] = successive_structural_level_drivers['emissions_intensity_driver_{}'.format(str(structure_variable_index))] * successive_structural_level_drivers['weighting_value_{}'.format(hierarchy_variable)]
                ...
                # sum the new successive_structural_level_drivers by year and mini_hierarchy_list
                successive_structural_level_drivers = successive_structural_level_drivers[[time_variable, 'emissions_intensity_driver_{}'.format(str(structure_variable_index)), 'energy_intensity_driver_{}'.format(str(structure_variable_index)), 'structural_driver_{}'.format(str(structure_variable_index))] + mini_hierarchy_list].groupby([time_variable] + mini_hierarchy_list).sum(numeric_only=True).reset_index()
    ...
    # separate the base year and other years
    emissions_base_year = mult_change[mult_change[time_variable] == base_year]
    emissions_other_years = mult_change[mult_change[time_variable] != base_year]
    # merge the two together
    mult_change = emissions_other_years.copy()
    mult_change['emissions_base_year'] = emissions_base_year[emissions_variable].values[0]
    # calc the ratio
    mult_change['emissions_mult_change'] = mult_change[emissions_variable] / mult_change['emissions_base_year']

    # merge the drivers with the ratio
    drivers_df = drivers_df.merge(mult_change[[time_variable, 'emissions_mult_change']], on=time_variable)

    # rename the columns
    drivers_df = drivers_df.rename(columns={'activity_driver': '{}'.format(activity_variable), 'first_structural_level_structural_driver': '{}'.format(structure_variables_list[0]), 'emissions_mult_change': 'Multiplicative change in {}'.format(emissions_variable)})

    for structure_variable in structure_variables_list[1:]:
        structure_variable_index = structure_variables_list.index(structure_variable)
        drivers_df = drivers_df.rename(columns={'emissions_intensity_driver_{}'.format(str(structure_variable_index)): '{} emissions intensity'.format(structure_variable), 'energy_intensity_driver_{}'.format(str(structure_variable_index)): '{} energy intensity'.format(structure_variable), 'structural_driver_{}'.format(str(structure_variable_index)): '{}'.format(structure_variable)})

    # add effect to the end of all names except 'Multiplicative change in {}'.format(emissions_variable) and Year
    drivers_df.columns = [col + ' effect' if col not in ['Multiplicative change in {}'.format(emissions_variable), time_variable] else col for col in drivers_df.columns]
    # for ease of use we didn't remove the intensity drivers for all structural variables except the last one. Now we will remove them.
    for structure_variable in structure_variables_list[1:-1]:
        drivers_df = drivers_df.drop(['{} energy intensity effect'.format(structure_variable)], axis=1)
        drivers_df = drivers_df.drop(['{} emissions intensity effect'.format(structure_variable)], axis=1)
    # and then move the last one to one from the end of the df by popping it then inserting it
    drivers_df.insert(len(drivers_df.columns) - 2, '{} emissions intensity effect'.format(structure_variables_list[-1]), drivers_df.pop('{} emissions intensity effect'.format(structure_variables_list[-1])))
    
    drivers_df.insert(len(drivers_df.columns) - 2, '{} energy intensity effect'.format(structure_variables_list[-1]), drivers_df.pop('{} energy intensity effect'.format(structure_variables_list[-1])))
    ...
    drivers_df['product'] = drivers_df[drivers_list].product(axis=1)
    drivers_df['difference'] = drivers_df['product'] - drivers_df['Multiplicative change in {}'.format(emissions_variable)]
    total_difference = drivers_df['difference'].sum().sum()
    if abs(total_difference) > 0.01:
        # breakpoint() # this started happening in 25th September. don't know why
        print('WARNING: The product of the hierarchical multiplicative drivers in each year for {} is not equal to the percent change in energy. The difference is {}. This seems to be a result of the input data, probably because the structure is inconsistent (i.e. certain structure variable categories are missing in certain years).'.format(extra_identifier, total_difference))
...
def convert_multiplicative_to_additive_emissions(config, multiplicative_output, emissions_data, activity_data, activity_variable, energy_variable, emissions_variable, time_variable, extra_identifier):
    ...
    base_year = emissions_data[time_variable].min()
    emissions_data_sum = emissions_data.groupby([time_variable])[emissions_variable].sum().reset_index()
    emissions_base_year = emissions_data_sum[emissions_data_sum[time_variable] == base_year][emissions_variable].values[0]
    activity_data_sum = activity_data.groupby([time_variable])[activity_variable].sum().reset_index()
    
    additive_output = additive_output.merge(emissions_data_sum, on=time_variable, how='outer')
    additive_output = additive_output.merge(activity_data_sum, on=time_variable, how='outer')
    ...
    additive_output['Additive change in {}'.format(emissions_variable)] = additive_output[drivers_list].sum(axis=1)
    
    additive_output['difference'] = additive_output['Additive change in {}'.format(emissions_variable)] - (additive_output[emissions_variable] - emissions_base_year)
    total_difference = additive_output['difference'].abs().sum()
    
    if total_difference > 0.01:
        print(f'WARNING: The sum of the hierarchical additive drivers in each year for {extra_identifier} is not equal to the total energy. The differences are {additive_output[[time_variable, emissions_variable, "difference"]].loc[additive_output["difference"] != 0]}')
    # Drop the unnecessary columns
    additive_output = additive_output.drop(columns=['difference', 'Multiplicative change in {}'.format(emissions_variable)])
    # Rename Activity to 'Total_{}'.format(activity_variable) and energy to 'Total {}'.format(emissions_variable)
    additive_output.rename(columns={emissions_variable: 'Total {}'.format(emissions_variable), activity_variable: 'Total_{}'.format(activity_variable)}, inplace=True)
