...
def hierarchical_LMDI_emissions(config, energy_data, emissions_data, activity_data, energy_variable, emissions_variable, activity_variable, structure_variables_list, time_variable, extra_identifier):
    ...
    for structure_variable in structure_variables_list:
        ...
        #TODO NOT SURE IF WE SHOULD BE INCLUDING ENERY INTENSITY HERE
        ##############
    ...
    for structure_variable in structure_variables_list[1:]:
        hierarchy_list.append(structure_variable)
        intersectoral_intensity_df = big_df[['log_mean_'+structure_variable,'emissions_intensity_log_mean_bot_{}'.format(structure_variable),  time_variable]+ hierarchy_list]
        intersectoral_intensity_df = intersectoral_intensity_df.drop_duplicates()
        intersectoral_intensity_df['weighting_value_'+structure_variable] = intersectoral_intensity_df['log_mean_'+structure_variable] / intersectoral_intensity_df['emissions_intensity_log_mean_bot_{}'.format(structure_variable)]

        #drop the columns we don't need
        intersectoral_intensity_df.drop(columns=['log_mean_'+structure_variable,'emissions_intensity_log_mean_bot_{}'.format(structure_variable)], inplace=True)
