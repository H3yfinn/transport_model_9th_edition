...
if plot_this:
    ...
    fig.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\macro_data_growth_indexes2.html')
    ...
    fig.update_yaxes(title_text='Index (base date = {}'.format(macro1['date'].min()))
    #save to plotting_output\input_analysis
    fig.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\macro_data_growth_indexes1.html')
...
activity = pd.read_csv(config.root_dir + '\\' + 'input_data\\from_8th\\reformatted\\activity_from_OSEMOSYS-hughslast.csv')
...
if plot_this:
    ...
    #save to plotting_output\input_analysis
    fig.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\macro_data_growth_analysis_no_china.html')
...
egeda_industry_output = pd.read_csv(config.root_dir + '\\' + 'D:\\APERC\\transport_data_system\\intermediate_data\\EGEDA\\egeda_industry_outputDATE20230522.csv')
...
region_mappings = pd.read_csv(config.root_dir + '\\' + 'D:\\APERC\\transport_model_9th_edition\\config\\concordances_and_config_data\\region_economy_mapping.csv')
...
if plot_this:
    for region in region_mappings.Region_growth_analysis.unique():
        ...
        #save to html
        fig.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\regions\\energy_use_cumulative_growth_vs_macro_growth_cumulative' + region + '.html')
        ...
        # #save to html
        # fig2.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\regions\\correlation_between_energy_use_cumulative_growth_rate_and_macro_growth_cumulative_' + region + '.html')
...
if plot_this:
    ...
    for economy in economies:
        ...
        #save to html   
        fig.write_html(config.root_dir + '\\' + 'plotting_output\\input_analysis\\economies\\correlation_heatmap_' + economy + '.html')
# %%
plot_this = False
if plot_this:
    #attempt heatmap from chatgpt:
