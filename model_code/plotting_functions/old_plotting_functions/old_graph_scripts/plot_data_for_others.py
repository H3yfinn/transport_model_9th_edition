...
#load data in
model_output_all = pd.read_csv(os.path.join(config.root_dir, 'output_data', 'model_output', config.model_output_file_name))
...
change_dataframe_aggregation = pd.read_csv(os.path.join(config.root_dir, 'intermediate_data', 'road_model', 'change_dataframe_aggregation.csv'))
...
plotly.offline.plot(fig, filename=os.path.join(config.root_dir, 'plotting_output', 'for_others', title + '.html'), auto_open=AUTO_OPEN_PLOTLY_GRAPHS)
