...
def plot_estimated_data_system_sales_share(config, sales, ECONOMY_ID):
    ...
    fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', 'transport_data_system_testing', '{}_{}_sales_share.html'.format(ECONOMY_ID,config.FILE_DATE_ID)))
    #it doesnt really matter what the values are since we'll be adjusting them as we get more data.

def plot_new_sales_shares(config, new_sales_shares_all, ECONOMY_ID):
    ...
    if plotting:
        ...
        new_sales_shares_all_plot['Vehicle_Transport_Medium'] = new_sales_shares_all_plot['Vehicle Type'] + '_' + new_sales_shares_all_plot['road']
        ...
        for scenario in new_sales_shares_all_plot['Scenario'].unique():
            for Vehicle_Transport_Medium in new_sales_shares_all_plot['Vehicle_Transport_Medium'].unique():
                ...
                fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', '{}_{}_{}_drive_share.html'.format(ECONOMY_ID, Vehicle_Transport_Medium, scenario), auto_open=False))
                ...
                #write to html in plotting_output\input_exploration\vehicle_sales_shares
                fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', '{}_{}_{}_Transport_type_share_pre_vehicle_share_adj.html'.format(ECONOMY_ID, Vehicle_Transport_Medium, scenario), auto_open=False))
        ...
        #replace road bool with 'road' or 'non_road'
        new_sales_shares_all_plot['road'] = new_sales_shares_all_plot['road'].replace({True:'road', False:'non_road'})
        new_sales_shares_all_plot['Vehicle_Transport_Medium'] = new_sales_shares_all_plot['Vehicle Type'] + '_' +  '_' + new_sales_shares_all_plot['road']
        for scenario in new_sales_shares_all_plot['Scenario'].unique():
            for transport_type in new_sales_shares_all_plot['Transport Type'].unique():
                plot_data = new_sales_shares_all_plot.loc[(new_sales_shares_all_plot['Scenario']==scenario) & (new_sales_shares_all_plot['Transport Type']==transport_type)].copy()
                ...
                #write to html in plotting_output\input_exploration\vehicle_sales_shares
                fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', 'by_economy', '{}_{}_{}_drive_share.html'.format(ECONOMY_ID, transport_type, scenario), auto_open=False))
                ...
                title = f'EV sales share, {transport_type}, {scenario}'
                #now plot
                fig = px.bar(plot_data, x='Date', y='Drive_share', color='Drive', facet_col='Vehicle Type', title=title, barmode='stack')
                
                fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', 'by_economy', 'stacked_{}_{}_{}_drive_share.html'.format(ECONOMY_ID, transport_type, scenario), auto_open=False))
                #############
                ...
                # #write to html in plotting_output\input_exploration\vehicle_sales_shares
                # fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', '{}_{}_Transport_type_share_pre_vehicke_share_adj.html'.format(Vehicle_Transport, scenario), auto_open=False)
    ...
    new_sales_shares_all_plot.to_csv(os.path.join(config.root_dir, 'output_data', 'assumptions_outputs', 'vehicle_sales_shares_{}_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID)), index=False)
        ...
        for scenario in new_sales_shares_all_plot_transport_type_shares['Scenario'].unique():
            for economy in new_sales_shares_all_plot_transport_type_shares['Economy'].unique():
                for medium in new_sales_shares_all_plot_transport_type_shares['road'].unique():
                    ...
                    fig = px.line(plot_data, x='Date', y='Value', color='Vehicle_drive', line_dash = 'Measure', facet_col='Transport Type',facet_col_wrap=1, title='Transport_type_share {}'.format(medium), markers=True)
                    #write to html in plotting_output\input_exploration\vehicle_sales_shares
                    fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', 'by_economy', '{}_{}_{}_Transport_type_share.html'.format(economy, scenario, medium), auto_open=False))
    print('Plots of new sales shares saved to plotting_output\\input_exploration\\vehicle_sales_shares\\')

def plot_input_sales_shares_before_interpolation(config, new_sales_shares_pre_interp):
    ...
    if plotting:
        ...
        for scenario in new_sales_shares_all_plot_transport_type_shares['Scenario'].unique():
            for economy in new_sales_shares_all_plot_transport_type_shares['Economy'].unique():
                ...
                #write to html in plotting_output/input_exploration/vehicle_sales_shares
                fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'vehicle_sales_shares', 'by_economy', f'{economy}_{scenario}drive_share_pre_interp.html'), auto_open=False)
    
    print('Plots of pre interpolation sales shares saved to plotting_output/input_exploration/vehicle_sales_shares/')
    
    
def plot_supply_side_fuel_mixing(config, supply_side_fuel_mixing, ECONOMY_ID, AUTO_OPEN=False):
    ...
    for scenario in supply_side_fuel_mixing_plot.Scenario.unique():
        scenario_data = supply_side_fuel_mixing_plot.loc[supply_side_fuel_mixing_plot['Scenario'] == scenario].copy()
        ...
        #save to html
        fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'fuel_mixing', f'{title}.html'), auto_open=AUTO_OPEN)

def plot_demand_side_fuel_mixing(config, demand_side_fuel_mixing, ECONOMY_ID):
    ...
    for scenario in demand_side_fuel_mixing_plot.Scenario.unique():
        ...
        title = 'Demand side fuel mixing for '+ECONOMY_ID + scenario + ' scenario'
        ...
        #save to html
        fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'fuel_mixing', f'{title}.html'))
        
def plot_average_intensity(config, non_road_model_input_wide):
    #grab economy id
    ...
    fig.write_html(os.path.join(config.root_dir, 'plotting_output', 'input_exploration', 'average_intensity', f'average_non_road_intensity_{economy_id}.html'), auto_open=False)
