...
def load_non_road_model_data(config, ECONOMY_ID, USE_ROAD_ACTIVITY_GROWTH_RATES_FOR_NON_ROAD):
    ...
    turnover_rate_midpoint_mult_adjustment_road_reference = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['TURNOVER_RATE_MIDPOINT_MULT_ADJUSTMENT_NON_ROAD_REFERENCE']
    turnover_rate_midpoint_mult_adjustment_road_target = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['TURNOVER_RATE_MIDPOINT_MULT_ADJUSTMENT_ROAD_TARGET']
...
def run_non_road_model(config, ECONOMY_ID, USE_ROAD_ACTIVITY_GROWTH_RATES_FOR_NON_ROAD = True, USE_COVID_RELATED_MILEAGE_CHANGE = True):
    output_file_name = config.root_dir + config.slash +'intermediate_data\\non_road_model\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name)
    ...
    # output_df.to_csv(config.root_dir + config.slash + 'a.csv', index=False)
...
#             EXPECTED_ENERGY_DECREASE_FROM_COVID_PASSENGER =  yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_ENERGY_DECREASE_FROM_COVID_PASSENGER']
#             X = EXPECTED_ENERGY_DECREASE_FROM_COVID_PASSENGER[economy]
#         elif transport_type =='freight':
#             EXPECTED_ENERGY_DECREASE_FROM_COVID_FREIGHT =  yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_ENERGY_DECREASE_FROM_COVID_FREIGHT']
