...
def run_road_model_for_year_y(config, year, previous_year_main_dataframe, main_dataframe, user_inputs_df_dict, growth_forecasts, change_dataframe_aggregation, low_ram_computer_files_list, low_ram_computer, previous_10_year_block, turnover_rate_parameters_dict, throw_error=False):
    ...
    # LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIED = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIED']
    ...
    for transport_type in change_dataframe['Transport Type'].unique():
        for scenario in change_dataframe['Scenario'].unique():
            ...
            YEARLY_EFFICIENCY_DEGRADATION_RATE = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['YEARLY_EFFICIENCY_DEGRADATION_RATE'][economy]
...
#         LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIED = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIEDPASSENGER']

#         EXPECTED_YEARS_TO_RETURN_TO_NORMAL_MILEAGE_FROM_COVID = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_YEARS_TO_RETURN_TO_NORMAL_MILEAGE_FROM_COVID_PASSENGER']
...
#         EXPECTED_ENERGY_DECREASE_FROM_COVID = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_ENERGY_DECREASE_FROM_COVID_PASSENGER']
#         X = EXPECTED_ENERGY_DECREASE_FROM_COVID[economy]
        
#     elif transport_type =='freight':
        
#         LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIED = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['LISTED_YEARS_WHEN_COVID_EFFECTS_APPLIEDFREIGHT']

#         EXPECTED_YEARS_TO_RETURN_TO_NORMAL_MILEAGE_FROM_COVID = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_YEARS_TO_RETURN_TO_NORMAL_MILEAGE_FROM_COVID_FREIGHT']
#         #There could be a number of years over which the decrease will be reverted, for which we will spread the increase over.
#         N = EXPECTED_YEARS_TO_RETURN_TO_NORMAL_MILEAGE_FROM_COVID[economy]
...
#         #load ECONOMIES_WITH_STOCKS_PER_CAPITA_REACHED from parameters.yml
#         EXPECTED_ENERGY_DECREASE_FROM_COVID = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['EXPECTED_ENERGY_DECREASE_FROM_COVID_FREIGHT']
#         X = EXPECTED_ENERGY_DECREASE_FROM_COVID[economy]
...
def prepare_road_model_inputs(config, road_model_input, ECONOMY_ID, low_ram_computer=True):
    ...
    if low_ram_computer:
        ...
        for file in glob.glob(os.path.join(config.root_dir + config.slash + 'intermediate_data\\main_dataframe_10_year_blocks\\', '*.csv')):
            os.remove(file)
    else:
...
def join_and_save_road_model_outputs(config, ECONOMY_ID, main_dataframe, low_ram_computer, low_ram_computer_files_list, change_dataframe_aggregation, first_model_run_bool):
    if first_model_run_bool:
        new_output_file = config.root_dir + config.slash + 'intermediate_data\\road_model\\first_run_{}_{}'.format(ECONOMY_ID, config.model_output_file_name)
    else:
        #this will be the name of the output file
        new_output_file = config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name)
    ...
    #save dataframe
    change_dataframe_aggregation.to_csv(config.root_dir + config.slash + f'intermediate_data\\road_model\\change_dataframe_aggregation_{ECONOMY_ID}.csv', index=False)
