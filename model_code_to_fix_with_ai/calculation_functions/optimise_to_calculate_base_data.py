...
def calculate_and_format_stocks_per_capita_constants(config, input_data_new_road, ECONOMY_ID):
    ...
    stocks_per_capita_factors = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_vehicles_per_stock_parameters.csv'.format(ECONOMY_ID))    
...
def load_in_optimisation_parameters(config, ECONOMY_ID):
    with open(config.root_dir + config.slash + 'config\\optimisation_parameters.yml') as file:
...
def optimise_to_find_base_year_values(config, input_data_new_road, ECONOMY_ID, methods, all_parameters_dicts, REMOVE_NON_MAJOR_VARIABLES, USE_MOVE_ELECTRICITY_USE_IN_ROAD_TO_RAIL_ESTO, SET_CNG_LPG_STOCKS_TO_ZERO=True):
    ...
    for method_and_params in methods_and_params:
        ...
        if SAVE_BOUNDS:
            ...
            df_transport.to_csv(config.root_dir + config.slash + 'intermediate_data\\analysis_single_use\\bounds.csv')
        ...
        if result.success:
            ...
            # # with open(config.root_dir + config.slash + 'inputs_to_objective_function_handler.pkl', 'rb') as f:
...
#     parameters = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)
...
def check_results_difference_after_optimisation(config, df_transport, df_transport_copy, results_dict, parameters_dict, economy, year, scenario, SAVE=True, BREAKPOINT=True, IGNORE_LARGE_ENERGY_RESIDUALS=False):
    ...
    if difference > sum_actual_energy * 0.1:# parameters_dict['tolerance_pct']:#testing 10% diff just to see  what soltuions looks like
        ...
        if SAVE:
            ...
            with open(config.root_dir + config.slash + f'plotting_output\\input_exploration\\results_dict_{economy}_{year}_{scenario}_{results_dict["file_id"]}_UNSUCCESSFUL.pkl', 'wb') as f:
                pickle.dump(results_dict, f)
            #and save the optimised data (df_transport)
            df_transport.to_csv(config.root_dir + config.slash + f'plotting_output\\input_exploration\\optimised_data_{economy}_{year}_{scenario}_{results_dict["file_id"]}_UNSUCCESSFUL.csv')
...
def plot_optimisation_results(config, optimised_data, input_data_new_road_df, results_dict, PLOT_NO_RESULTS=False):
    ...
    if not NO_RESULTS:
        try:
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\optimisation_reestimations\\{results_dict["Economy"]}_{results_dict["Date"]}_{results_dict["method"]}_{param_id}.html')
            ...
            df_all_pct_diff_eds.to_csv(config.root_dir + config.slash + f'plotting_output\\input_exploration\\optimisation_reestimations\\{results_dict["Economy"]}_{results_dict["Date"]}_{results_dict["method"]}_{param_id}.csv')
        except:
    ...
    fig.update_yaxes(matches=None)
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\optimisation_reestimations\\{results_dict["Economy"]}_{results_dict["Date"]}_{results_dict["method"]}_{param_id}_energy_comparison.html')
    #write the data to a csv so we can inspect it if it seems fishy 
    df_all_long_eds.to_csv(config.root_dir + config.slash + f'plotting_output\\input_exploration\\optimisation_reestimations\\{results_dict["Economy"]}_{results_dict["Date"]}_{results_dict["method"]}_{param_id}_energy_comparison.csv')
...
def optimisation_handler_testing(config, ECONOMY_ID, input_data_new_road_df=None, SAVE_ALL_RESULTS=True, SAVE_INDIVIDUAL_RESULTS=False, REMOVE_NON_MAJOR_VARIABLES=True, USE_MOVE_ELECTRICITY_USE_IN_ROAD_TO_RAIL_ESTO=True, USE_SAVED_OPT_PARAMATERS=False, parameters_ranges=None, methods_list=None, PARAMETERS_RANGES_KEY='ALL'):
    ...
    for parameters_dict in all_params:
        for method in methods_list:
            if input_data_new_road_df is None:
                input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_{ECONOMY_ID}.pkl')
            else:
                input_data_new_road = input_data_new_road_df.copy()
            ...
            if SAVE_INDIVIDUAL_RESULTS:
                ...
                pickle.dump(results_dict, open(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\results_dict_{ECONOMY_ID}_{file_id}_{config.transport_data_system_FILE_DATE_ID}.pkl', 'wb'))
                optimised_data.to_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\optimised_data_{ECONOMY_ID}_{file_id}_{config.transport_data_system_FILE_DATE_ID}.pkl')             
    ...
    if SAVE_ALL_RESULTS:
        file_id = datetime.datetime.now().strftime("%Y%m%d_%H%M") 
        all_outputs.to_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\all_outputs_{ECONOMY_ID}_{file_id}_{config.transport_data_system_FILE_DATE_ID}.pkl')
        pickle.dump(all_results_dicts, open(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\all_results_dicts_{ECONOMY_ID}_{file_id}_{config.transport_data_system_FILE_DATE_ID}.pkl', 'wb'))
...
def optimisation_handler(config, input_data_new_road, SAVE_ALL_RESULTS=False, method='L-BFGS-B', PLOT=False, REMOVE_NON_MAJOR_VARIABLES=True, USE_MOVE_ELECTRICITY_USE_IN_ROAD_TO_RAIL_ESTO=True, USE_SAVED_OPT_PARAMATERS=False, parameters_ranges = None, methods_list=None, PARAMETERS_RANGES_KEY='ALL'):
    ...
    file_id = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    input_data_new_road.to_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_{ECONOMY_ID}_{config.FILE_DATE_ID}_{config.transport_data_system_FILE_DATE_ID}.pkl')
    #note that energy_new is the energy we want to optimise for, to achive. energy old is what  we have using the current values (stocks, mileage, eff etc).
    ...
    if optimised_data is None:
        ...
        input_data_new_road.to_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\failed_run_input_data_new_road_{ECONOMY_ID}_{file_id}_{config.transport_data_system_FILE_DATE_ID}.pkl')
    ...
    optimised_data.to_pickle(config.root_dir + config.slash + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{config.FILE_DATE_ID}_{config.transport_data_system_FILE_DATE_ID}.pkl')         
...
def save_and_overwrite_parameters_in_yaml(config, ECONOMY_ID, results_dict):
    ...
    if parameters_dict is not None:
        if ECONOMY_ID in parameters_dict.keys():
            ...
            with open(config.root_dir + config.slash + f'config\\archive\\optimisation_parameters_{ECONOMY_ID}_{config.FILE_DATE_ID}_{config.transport_data_system_FILE_DATE_ID}.yml', 'w') as file:
    ...
    parameters_dict[ECONOMY_ID]['method'] = results_dict['method']
    #save
    with open(config.root_dir + config.slash + 'config\\optimisation_parameters.yml', 'w') as file:
...
def plot_data_from_saved_results(config, ECONOMY_ID, FILE_DATE_ID=None, FILE_DATE_ID_MIN_HOURS=None, BY_FILE_NAME=False, optimised_data_filename=None, input_data_new_road_filename=None, results_dict_filename=None):
    ...
    if BY_FILE_NAME:
        ...
        input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_{ECONOMY_ID}_{FILE_DATE_ID}_{config.transport_data_system_FILE_DATE_ID}.pkl')

        optimised_data = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\optimised_data_{ECONOMY_ID}_{FILE_DATE_ID_MIN_HOURS}_{config.transport_data_system_FILE_DATE_ID}.pkl')

        results_dict = pickle.load(open(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\results_dict_{ECONOMY_ID}_{FILE_DATE_ID_MIN_HOURS}_{config.transport_data_system_FILE_DATE_ID}.pkl', 'rb'))
        plot_optimisation_results(config, optimised_data, input_data_new_road, results_dict=results_dict)
    elif FILE_DATE_ID is not None:
        for file in os.listdir(config.root_dir + config.slash + 'intermediate_data\\analysis_single_use\\'):
        ...
        input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_{ECONOMY_ID}_{FILE_DATE_ID}_{config.transport_data_system_FILE_DATE_ID}.pkl')
        optimised_data = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\opti    mised_data_{ECONOMY_ID}_{FILE_DATE_ID_MIN_HOURS}_{config.transport_data_system_FILE_DATE_ID}.pkl')
        results_dict = pickle.load(open(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\results_dict_{ECONOMY_ID}_{FILE_DATE_ID_MIN_HOURS}_{config.transport_data_system_FILE_DATE_ID}.pkl', 'rb'))
        #breakpoint()
        plot_optimisation_results(config, optimised_data, input_data_new_road, results_dict=results_dict)
...
# ECONOMY_ID = '08_JPN'
    ...
    # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_08_JPN_20240611_DATE20240605.pkl')
    
    # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_15_PHL_20240530.pkl')
    
    # input_data_new_road_actual_run_01_AUS_20240311_DATE20240304_DATE20240215
    
    # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_01_AUS_20240311_DATE20240304_DATE20240215.pkl')
    # # # # # # # # # # # # # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\failed_run_input_data_new_road_05_PRC_20231120_1341_DATE20231106.pkl')
    # # # # # # # # # # # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\input_data_new_road_actual_run_21_VN_20231128_DATE20231106.pkl')
    # # # # # # # # # # # # # # input_data_new_road = pd.read_pickle(config.root_dir + config.slash + f' \\analysis_single_use\\failed_run_input_data_new_road_20_USA_20231010HYUJ7NADZXQE 1613_DATE20231010_DATE20231010.pkl')
...
#%%
#%%
# results_dict = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\analysis_single_use\\results_dict_15_PHL_20240105_1154_DATE20231213.pkl')
# %%
