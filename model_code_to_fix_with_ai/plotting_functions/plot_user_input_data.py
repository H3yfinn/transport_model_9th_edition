...
def plot_estimated_data_system_sales_share(config, sales, ECONOMY_ID):
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\vehicle_sales_shares\\transport_data_system_testing\\{}_{}_sales_share.html'.format(ECONOMY_ID,config.FILE_DATE_ID))
...
def plot_new_sales_shares(config, new_sales_shares_all, ECONOMY_ID):
    ...
    if plotting:
        ...
        for scenario in new_sales_shares_all_plot['Scenario'].unique():
            for Vehicle_Transport_Medium in new_sales_shares_all_plot['Vehicle_Transport_Medium'].unique():
                ...
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\{ECONOMY_ID}_{Vehicle_Transport_Medium}_{scenario}_drive_share.html', auto_open=False)
                ...
                #write to html in plotting_output\input_exploration\vehicle_sales_shares
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\{ECONOMY_ID}_{Vehicle_Transport_Medium}_{scenario}Transport_type_share_pre_vehicle_share_adj.html', auto_open=False)
        ...
        for scenario in new_sales_shares_all_plot['Scenario'].unique():
            for transport_type in new_sales_shares_all_plot['Transport Type'].unique():
                ...
                #write to html in plotting_output\input_exploration\vehicle_sales_shares
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\by_economy\\{ECONOMY_ID}_{transport_type}_{scenario}_drive_share.html', auto_open=False)
                ...
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\by_economy\\stacked_{ECONOMY_ID}_{transport_type}_{scenario}_drive_share.html', auto_open=False)
                #############
                ...
                # #write to html in plotting_output\input_exploration\vehicle_sales_shares
                # fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\{Vehicle_Transport}_{scenario}Transport_type_share_pre_vehicke_share_adj.html', auto_open=False)
    ...
    new_sales_shares_all_plot.to_csv(config.root_dir + config.slash + f'output_data\\assumptions_outputs\\vehicle_sales_shares_{ECONOMY_ID}_{config.FILE_DATE_ID}.csv', index=False)
        ...
        for scenario in new_sales_shares_all_plot_transport_type_shares['Scenario'].unique():
            for economy in new_sales_shares_all_plot_transport_type_shares['Economy'].unique():
                for medium in new_sales_shares_all_plot_transport_type_shares['road'].unique():
                    ...
                    #write to html in plotting_output\input_exploration\vehicle_sales_shares
                    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\by_economy\\{economy}_{scenario}_{medium}_Transport_type_share.html', auto_open=False)
    ...
    if plotting:
        ...
        for scenario in new_sales_shares_all_plot_transport_type_shares['Scenario'].unique():
            for economy in new_sales_shares_all_plot_transport_type_shares['Economy'].unique():
                ...
                #write to html in plotting_output\input_exploration\vehicle_sales_shares
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\vehicle_sales_shares\\by_economy\\{economy}_{scenario}drive_share_pre_interp.html', auto_open=False)
...
def plot_supply_side_fuel_mixing(config, supply_side_fuel_mixing, ECONOMY_ID, AUTO_OPEN=False):
    ...
    for scenario in supply_side_fuel_mixing_plot.Scenario.unique():
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash + f"plotting_output\\input_exploration\\fuel_mixing\\{title}.html", auto_open=AUTO_OPEN)
...
def plot_demand_side_fuel_mixing(config, demand_side_fuel_mixing, ECONOMY_ID):
    ...
    for scenario in demand_side_fuel_mixing_plot.Scenario.unique():
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash + f"plotting_output\\input_exploration\\fuel_mixing\\{title}.html")
...
def plot_average_intensity(config, non_road_model_input_wide):
    ...
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\average_intensity\\average_non_road_intensity_{economy_id}.html', auto_open=False)
