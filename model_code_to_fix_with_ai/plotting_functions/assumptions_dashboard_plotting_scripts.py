...
def write_graph_to_html(config, filename, graph_type, plot_data, economy, x=None, y=None, color=None, title=None, line_dash=None, y_axes_title=None, legend_title=None, font_size=30, marker_line_width=2.5, line_width=10, colors_dict={}):
    ...
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\{economy}\\transport_type_intensity_analysis\\' + filename)
...
def prodcue_LMDI_mutliplicative_plot(config, ECONOMY_IDs, fig_dict, colors_dict, transport_type, medium, WRITE_HTML=True):
    ...
    for scenario in config.economy_scenario_concordance.Scenario.unique():
        for economy in ECONOMY_IDs:
            ...
            try:
                lmdi_data = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\LMDI\\{economy}\\{file_identifier}.csv')
            except:
...
def plot_stocks_per_capita(config, ECONOMY_IDs, gompertz_parameters_df, model_output_detailed, first_road_model_run_data, fig_dict, color_preparation_list, colors_dict, PLOT_ANYWAY=True, WRITE_HTML=True):
    ...
    ECONOMIES_WITH_MAX_STOCKS_PER_CAPITA_REACHED = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['ECONOMIES_WITH_MAX_STOCKS_PER_CAPITA_REACHED']
    ...
    # else:
    vehicles_per_stock_parameters = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_vehicles_per_stock_parameters.csv'.format(ECONOMY_IDs[0]))
...
def emissions_by_fuel_type(config, ECONOMY_IDs, emissions_factors, model_output_with_fuels_df, fig_dict, color_preparation_list, colors_dict, transport_type, USE_AVG_GENERATION_EMISSIONS_FACTOR=True, USE_CUM_SUM_OF_EMISSIONS=False, WRITE_HTML=True):
    ...
    if USE_AVG_GENERATION_EMISSIONS_FACTOR:
        ...
        emissions_factor_elec = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\outlook_8th_emissions_factors_with_electricity.csv')#c:\Users\finbar.maunsell\github\aperc-emissions\output_data\outlook_8th_emissions_factors_with_electricity.csv
...
def plot_energy_efficiency_timeseries(config, ECONOMY_IDs, model_output_detailed, fig_dict, DROP_NON_ROAD_TRANSPORT, color_preparation_list, colors_dict, transport_type, extra_ice_line=True, extra_bev_line=True, vehicle_type_grouping='simplified', WRITE_HTML=True):
    PLOTTED=True
    conversion_factors = pd.read_csv(config.root_dir + config.slash + 'config\\concordances_and_config_data\\conversion_factors.csv')
...
def plot_turnover_rate_age_curve(config, ECONOMY_IDs, model_output_detailed_df, fig_dict, colors_dict, WRITE_HTML=True):
    ...
    turnover_rate_parameters_dict = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['turnover_rate_parameters_dict']
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        for economy in ECONOMY_IDs:
            ...
            if scenario == 'Reference':
                turnover_rate_midpoint_mult_adjustment_road = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['TURNOVER_RATE_MIDPOINT_MULT_ADJUSTMENT_ROAD_REFERENCE']
            elif scenario == 'Target':
                turnover_rate_midpoint_mult_adjustment_road = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['TURNOVER_RATE_MIDPOINT_MULT_ADJUSTMENT_ROAD_TARGET']
            else:
...
def plot_lca_inputs(config, lca_inputs):
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\lifecycle_emissions\\lifecycle_emissions_of_cars_inputs.html')
...
def plot_emissions_from_use_for_single_vehicle(config, emissions_factors, fuels_to_plot):
    ...
    for scenario in emissions_factors_filtered.Scenario.unique():
        ...
        #drop the legend
        fig.update_layout(showlegend=False)
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\lifecycle_emissions\\{economy}_lifecycle_emissions_LINE_{scenario}.html')
...
def extract_lifecycle_emissions_series(config):
    ...
    lca = pd.read_excel(config.root_dir + config.slash + 'input_data\\lifecycle_emissions.xlsx')
...
def plot_new_vehicle_emissions_intensity_by_vehicle_type(config, fig_dict, ECONOMY_IDs, model_output_detailed_df, emissions_factors, colors_dict, color_preparation_list, DROP_NON_ROAD_TRANSPORT=True, transport_type='all', extra_ice_line=True, extra_bev_line=True, WRITE_HTML=True, vehicle_type_grouping='simplified', USE_AVG_GENERATION_EMISSIONS_FACTOR=False):
    ...
    drive_type_to_fuel = pd.read_csv(config.root_dir + config.slash + 'config\\concordances_and_config_data\\drive_type_to_fuel.csv')
...
def plot_number_of_stocks_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, model_output_detailed_df, colors_dict, transport_type, SALES, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL):
    ...
    for scenario in stocks['Scenario'].unique():
        ...
        if transport_type == 'passenger':
            ...
            #save to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\{sales_or_stocks}_{scenario}_{economy_grouping_name}_passenger.html')
        elif transport_type == 'freight':
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\{sales_or_stocks}_{scenario}_{economy_grouping_name}_freight.html')
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\{sales_or_stocks}_{scenario}_{economy_grouping_name}_all.html')
        else:
...
def plot_share_of_vehicle_type_by_transport_type_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, new_sales_shares_all_plot_drive_shares_df, stocks_df, model_output_detailed_df, colors_dict, share_of_transport_type_type, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL, INCLUDE_OTHER_DRIVES=True):
    ...
    for scenario in new_sales_shares_all_plot_drive_shares['Scenario'].unique():
        ...
        if share_of_transport_type_type == 'passenger':
            ...
            #save to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_passenger{extra_identifier}.html')
        elif share_of_transport_type_type == 'freight':
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_freight{extra_identifier}.html')
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_vehicle_type_by_transport_type_{scenario}_{economy_grouping_name}_all{extra_identifier}.html')
        else:
            raise ValueError('share_of_transport_type_type must be either passenger or freight')
    return
...
def plot_share_transport_type_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, new_sales_shares_all_plot_drive_shares_df, stocks_df, model_output_detailed_df, colors_dict, share_of_transport_type_type, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL, INCLUDE_OTHER_DRIVES=True):
    ...
    for scenario in new_sales_shares_all_plot_drive_shares['Scenario'].unique():
        ...
        if share_of_transport_type_type == 'passenger':
            ...
            fig = px.line(plot_data[plot_data['Transport Type']=='passenger'], x='Date', y='Value', color='Drive', title=title, line_dash='line_dash', color_discrete_map=colors_dict, facet_col='Economy', facet_col_wrap=7)
            #save to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_transport_type_{scenario}_{economy_grouping_name}_passenger{extra_identifier}.html')
        elif share_of_transport_type_type == 'freight':
            title = f'Shares for freight (%) - {economy_grouping_name} - {scenario}'

            fig = px.line(plot_data[plot_data['Transport Type']=='freight'], x='Date', y='Value', color='Drive', title=title, line_dash='line_dash', color_discrete_map=colors_dict, facet_col='Economy', facet_col_wrap=7)
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_transport_type_{scenario}_{economy_grouping_name}_freight{extra_identifier}.html')
            ...
            fig = px.line(plot_data_all, x='Date', y='Value', color='Drive', title=title, line_dash='line_dash', color_discrete_map=colors_dict, facet_col='Economy', facet_col_wrap=7)
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\shares_of_transport_type_{scenario}_{economy_grouping_name}_all{extra_identifier}.html')
        else:
            raise ValueError('share_of_transport_type_type must be either passenger or freight')
    return
...
def plot_supply_side_fuel_mixing_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, supply_side_fuel_mixing_df, supply_side_fuel_mixing_output, colors_dict, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\supply_side_fuel_mixing_{scenario}_{economy_grouping_name}{extra_identifier}.html')
...
def energy_use_by_fuel_type_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, energy_output_for_outlook_data_system_tall_df, colors_dict, transport_type, medium, INDEPENDENT_AXIS, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        if transport_type=='passenger':
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\energy_use_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_{medium}{extra_identifier}.html')
            if INDEPENDENT_AXIS and not ONLY_AGG_OF_ALL:
                fig.update_yaxes(matches=None)
                fig.for_each_yaxis(lambda yaxis: yaxis.update(showticklabels=True))
                #write to html
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\energy_use_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_{medium}_indep_axis.html')
...
def emissions_by_fuel_type_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, emissions_factors, model_output_with_fuels_df, colors_dict, transport_type, INDEPENDENT_AXIS, AGG_OF_ALL_ECONOMIES, USE_AVG_GENERATION_EMISSIONS_FACTOR=False, ONLY_AGG_OF_ALL=False):
    ...
    for scenario in config.economy_scenario_concordance['Scenario'].unique():
        ...
        if transport_type=='passenger':
            ...
            #save to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\emissions_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}{extra_identifier}.html')
            ...
            if INDEPENDENT_AXIS:
                ...
                #save to html
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\emissions_by_fuel_type_{scenario}_{economy_grouping_name}_{transport_type}_indep_axis.html')
...
def share_of_emissions_by_vehicle_type_FOR_MULTIPLE_ECONOMIES(config, economy_grouping_name, emissions_factors, model_output_with_fuels_df, colors_dict, AGG_OF_ALL_ECONOMIES, USE_AVG_GENERATION_EMISSIONS_FACTOR=True, ONLY_AGG_OF_ALL=False):
    ...
    for scenario in emissions_by_vehicle_type['Scenario'].unique():
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\share_of_emissions_by_vehicle_type_{scenario}_{economy_grouping_name}{extra_identifier}.html')
    return
...
def prodcue_LMDI_mutliplicative_plot_FOR_MULTIPLE_ECONOMIES(config,  economy_grouping_name, model_output_with_fuels, colors_dict, ECONOMY_IDs,  transport_type, medium, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL):
    ...
    for scenario in config.economy_scenario_concordance.Scenario.unique():
        ...
        for economy in ECONOMY_IDs:
            ...
            try:
                if economy == 'all':
                    ...
                    lmdi_data_economy = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\LMDI\\APEC\\{file_identifier}.csv')
                else:
                    lmdi_data_economy = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\LMDI\\{economy}\\{file_identifier}.csv')
            except:
                if config.PRINT_WARNINGS_FOR_FUTURE_WORK:
                    breakpoint()
                continue
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\lmdi_multiplicative_{scenario}_{economy_grouping_name}{extra_identifier}.html')
...
def produce_LMDI_additive_plot_FOR_MULTIPLE_ECONOMIES(config,  economy_grouping_name, model_output_with_fuels, colors_dict, ECONOMY_IDs, medium, AGG_OF_ALL_ECONOMIES, ONLY_AGG_OF_ALL):
    ...
    for scenario in config.economy_scenario_concordance.Scenario.unique():
        ...
        #save to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\dashboards\\multiple_economy_dashboards\\lmdi_additive_{scenario}_{economy_grouping_name}{extra_identifier}.html')
