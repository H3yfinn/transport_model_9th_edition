...
def transfer_growth_between_mediums(config, model_output_all, ECONOMY_ID):
    ...
    SHIFTED_YEARLY_GROWTH_RATE_FROM_MEDIUM_TO_MEDIUM_DICT = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml', 'r'), Loader=yaml.FullLoader)['SHIFTED_YEARLY_GROWTH_RATE_FROM_MEDIUM_TO_MEDIUM_DICT']
    ...
    activity_change_for_plotting_all.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_outputs\\{}_medium_to_medium_activity_change_for_plotting{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID), index=False)
    return model_output_all
