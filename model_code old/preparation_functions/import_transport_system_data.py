...
def import_transport_system_data(config):
    ...
    model_concordances_measures = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_base_year_measures_file_name))
    ...
    transport_data_system_df = pd.read_csv(config.root_dir + config.slash + '{}\\combined_data_{}.csv'.format(transport_data_system_folder,config.transport_data_system_FILE_DATE_ID))
    ...
    TRANSPORT_DATA_SYSTEM_DATE_TO_USE_FOR_NON_ROAD_TRANSPORT_TYPE_SPLITS = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['TRANSPORT_DATA_SYSTEM_DATE_TO_USE_FOR_NON_ROAD_TRANSPORT_TYPE_SPLITS']
    ...
    if missing_index_values1.empty:
        ...
        'Transport Type', 'Drive'], inplace=True)
            ...
            save_this = True
            if save_this:
              a.to_csv(config.root_dir + config.slash + 'intermediate_data\\transport_data_system\\missing_important_values.csv')
    ...
    if not missing_index_values1.empty:
        ...
        missing_index_values1.to_csv(config.root_dir + config.slash + 'output_data\\for_other_modellers\\missing_values\\{}_missing_input_values.csv'.format(config.FILE_DATE_ID), index=False)
    else:
    ...
    new_transport_data_system_df.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\transport_data_system_extract.csv', index=False)
...
def adjust_non_road_TEMP(config, transport_data_system_df, model_concordances_measures, TRANSPORT_DATA_SYSTEM_DATE_TO_USE_FOR_NON_ROAD_TRANSPORT_TYPE_SPLITS, SINGLE_ECONOMY=None, RAISE_ERROR=False):
    ...
    # date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + '..\transport_data_system\intermediate_data\EGEDA\', 'model_input_9th_cleaned')
    # esto_non_road = pd.read_csv(config.root_dir + config.slash +f'..\\transport_data_system\\intermediate_data\\EGEDA\\model_input_9th_cleanedDATE{date_id}.csv')
    ...
    supply_side_fuel_mixing_fuels = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_supply_side_fuel_mixing_file_name), dtype={'Demand_side_fuel_mixing': str}).New_fuel.unique().tolist()
    # #TEMP
