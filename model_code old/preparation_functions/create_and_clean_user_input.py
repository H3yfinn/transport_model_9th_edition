...
def create_and_clean_user_input(config, ECONOMY_ID, ADVANCE_BASE_YEAR_TO_OUTLOOK_BASE_YEAR=False):
    ...
    # for file in os.listdir(config.root_dir + config.slash + f'input_data\\user_input_spreadsheets\\{ECONOMY_ID}'):
    #     #check its a csv
    #     if file[-4:] != '.csv':
    #         continue
    #     print(f'Importing user input file: {file}')
    #     user_input = pd.concat([user_input, pd.read_csv(config.root_dir + config.slash +f'input_data\\user_input_spreadsheets\\{ECONOMY_ID}\\{file}')])
    
    #laod concordances for checking
    model_concordances_user_input_and_growth_rates = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_user_input_and_growth_rates_file_name))#seems we're missing ghompertz hbere?
    ...
    if (user_input[user_input.Data_available == 'row_and_data_not_available'].Date.max() > 2050) or (user_input[user_input.Data_available == 'data_not_available'].Date.max() > 2050):
        ...
        if len(user_input_new[user_input_new.Value.isna()]) >0:
            ...
            #save them to csv
            user_input_new_nas.to_csv(config.root_dir + config.slash + 'intermediate_data\\errors\\user_input_new_nas.csv', index=False)
    ...
    ECONOMY_BASE_YEARS_DICT = yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['ECONOMY_BASE_YEARS_DICT']
    ...
    user_input_new.to_csv(config.root_dir + config.slash +f'intermediate_data\\model_inputs\\{ECONOMY_ID}_user_inputs_and_growth_rates.csv', index=False)
...
def extract_economy_data_from_user_input_spreadsheets(config, ECONOMY_ID):
    ...
    for file in os.listdir(config.root_dir + config.slash + f'input_data\\user_input_spreadsheets\\'):
        #check its a csv
        if file[-4:] != '.csv':
            continue
        user_input = pd.read_csv(config.root_dir + config.slash +f'input_data\\user_input_spreadsheets\\{file}')
        ...
        model_concordances_user_input_and_growth_rates_original= pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_user_input_and_growth_rates_file_name))
