...
#PLEASE NOTE THAT ALL MODEL CONCORDANCE FILE NAMES VARIABLES ARE STORED AND SET IN ./code/config.py
...
def create_all_concordances(config, USE_LATEST_CONCORDANCES=False):
    ...
    if check_for_and_save_previous_concordances(config, USE_LATEST_CONCORDANCES):
        return
    manually_defined_transport_categories = pd.read_csv(config.root_dir + config.slash + 'config\\concordances_and_config_data\\manually_defined_transport_categories.csv')
    ...
    # if not os.path.exists(config.root_dir + config.slash + 'config\\concordances_and_config_data\\computer_generated_concordances'):
        # os.makedirs('config\\concordances_and_config_data\\computer_generated_concordances')
    model_concordances.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_file_name), index=False)
    ...
    drive_type_to_fuel_df = pd.read_csv(config.root_dir + config.slash + 'config\\concordances_and_config_data\\drive_type_to_fuel.csv')
    ...
    #save
    model_concordances_fuels.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_file_name_fuels), index=False)
    model_concordances_fuels_NO_BIOFUELS.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_file_name_fuels_NO_BIOFUELS), index=False)
    ...
    measure_to_unit_concordance_copy = config.measure_to_unit_concordance[['Measure', 'Unit']].copy()

    #merge the dict to our model concordances
    model_concordances_BASE_YEAR_measures = model_concordances_BASE_YEAR_measures.merge(measure_to_unit_concordance_copy, how='left', on=['Measure'])
    ...
    #now save
    model_concordances_BASE_YEAR_measures.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_base_year_measures_file_name), index=False)
    ...
    model_concordances_user_input_and_growth_rates.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_user_input_and_growth_rates_file_name), index=False)
...
def create_fuel_mixing_concordances(config, model_concordances_fuels):
    ...
    if (demand['_merge'] == 'right_only').any():
        breakpoint()
        missing = demand[demand['_merge'] == 'right_only']['Drive'].unique()
        raise ValueError('There are fuels in the drive_type_to_fuel_df that are not in the model_concordances_fuels. Please check the drive_type_to_fuel_df and model_concordances_fuels - note this could be because you should add the rows manually (unfortunately) to model_concordances_fuels. {}'.format(missing))
    ...
    if (supply['_merge'] != 'both').any():
        breakpoint()
        missing = supply[supply['_merge'] != 'both']['Drive'].unique()
        raise ValueError('There are fuels in the drive_type_to_fuel_df that are not in the model_concordances_fuels. Please check the drive_type_to_fuel_df and model_concordances_fuels - note this could be because you should add the rows manually (unfortunately) to model_concordances_fuels: {}'.format(missing))
    #drop _merge col
    ...
    #save
    demand.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_demand_side_fuel_mixing_file_name), index=False)
    supply.to_csv(config.root_dir + config.slash + 'intermediate_data\\computer_generated_concordances\\{}'.format(config.model_concordances_supply_side_fuel_mixing_file_name), index=False)
