...
def import_macro_data(config, UPDATE_INDUSTRY_VALUES, PLOT=False):
    ...
    macro = pd.read_csv(config.root_dir + config.slash +f'input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv')
    ...
    growth_coeff = pd.read_csv(config.root_dir + config.slash + 'input_data\\growth_coefficients_by_region.csv')
    ...
    activity_growth_8th = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\reformatted\\activity_growth_8th.csv')
    ...
    macro3.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\regression_based_growth_estimates.csv', index=False)
...
def plot_png_growth(config, png_growth):
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\png_growth_rates.html', auto_open=True) 
...
def tie_freight_growth_to_gdp_growth(config, macro1, UPDATE_INDUSTRY_VALUES):
    ...
    # freight_to_gdp_growth_ratio = pd.read_excel(config.root_dir + config.slash + 'input_data\\parameters.xlsx', sheet_name='freight_to_gdp_growth_ratio')
    ...
    NON_INDUSTRY_FREIGHT_ADDITION =  yaml.load(open(config.root_dir + config.slash + 'config\\parameters.yml'), Loader=yaml.FullLoader)['NON_INDUSTRY_FREIGHT_ADDITION']
...
def grab_gdp_shares_from_industry(config, UPDATE_INDUSTRY_VALUES):
    ...
    for economy in config.economy_scenario_concordance['Economy'].unique():
        ...
        industry_path_onedrive = root_onedrive  + config.slash + economy + config.slash + industry_filename
        industry_path_local = config.root_dir + config.slash +  root_local + industry_filename
        manu_path_onedrive = root_onedrive + config.slash + economy + config.slash + manu_filename
        manu_path_local = config.root_dir + config.slash + root_local + manu_filename
        ...
        if UPDATE_INDUSTRY_VALUES:
            ...
            try:
                ...
                if economy in ['15_PHL', '17_SGP']:
                    ...
                    alt_ind_filepath = root_onedrive +  alt_econ_name + config.slash + f'{alt_econ_name}_NV.IND.TOTL.ZS.csv'
                    alt_manu_filepath = root_onedrive +  alt_econ_name + config.slash + f'{alt_econ_name}_NV.IND.MANF.ZS.csv'
                    try:
    ...
    all_shares.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\industry_gdp_shares.csv', index=False)
    ...
    if UPDATE_INDUSTRY_VALUES:
        ...
        fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\industry_gdp_shares.html', auto_open=True)
...
def calculate_VN_share(config, all_shares):
    ...
    for economy in sea_economies:
        df = pd.read_csv(config.root_dir + config.slash +f'input_data\\macro\\industry_gdp_shares\\{economy}_NV.IND.MANF.ZS.csv').rename(columns={'value': 'manu_share', 'year':'Date'})
