...
def communicate_missing_input_data(config):
    aggregated_model_data = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\aggregated_model_data.csv'.format(config.FILE_DATE_ID))
    ...
    if plot_colors_to_use:
        ...
        plt.savefig(config.root_dir + config.slash + 'plotting_output\\plot_data_coverage\\{}_colors_to_use.png'.format(config.FILE_DATE_ID))
        plt.close()
        ...
        for measure in aggregated_model_data['Measure'].unique():
            for transport_type in aggregated_model_data['Transport Type'].unique():
                ...
                if number_of_values == 0:
                    plt.savefig(config.root_dir + config.slash + 'plotting_output\\plot_data_coverage\\NOVALUES_{}_{}_{}_{}_plot.png'.format(config.FILE_DATE_ID, measure, transport_type, scenario))
                else:
                    plt.savefig(config.root_dir + config.slash + 'plotting_output\\plot_data_coverage\\{}_{}_{}_{}_plot.png'.format(config.FILE_DATE_ID, measure, transport_type, scenario))
    ...
    user_input = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\user_inputs_and_growth_rates.csv')
