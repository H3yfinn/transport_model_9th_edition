...
def create_archiving_folder_for_FILE_DATE_ID(config):
    ...
    if config.FILE_DATE_ID == '':
        archive_folder_name = config.root_dir + config.slash + 'input_data\\previous_run_archive\\latest_test_run'
    else:
        archive_folder_name = config.root_dir + config.slash + 'input_data\\previous_run_archive\\' + config.FILE_DATE_ID
        ...
        if os.path.exists(archive_folder_name):
            ...
            archive_folder_name = config.root_dir + config.slash + 'input_data\\previous_run_archive\\' + config.FILE_DATE_ID +config.slash+ new_FILE_DATE_ID
        
            if not os.path.exists(archive_folder_name):
                os.mkdir(archive_folder_name)
        else:
            if not os.path.exists(config.root_dir + config.slash + 'input_data\\previous_run_archive\\'):
                os.mkdir(config.root_dir + config.slash + 'input_data\\previous_run_archive\\')
            os.mkdir(archive_folder_name)
...
def archive_lots_of_files(config, archive_folder_name):
    ...
    #load output data
    model_output_detailed = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}'.format(config.model_output_file_name))
    model_output_non_detailed = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output\\{}'.format(config.model_output_file_name))
    model_output_all_with_fuels = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}'.format(config.model_output_file_name))

    #save output data
    model_output_detailed.to_csv(config.root_dir + config.slash + '{}\\model_output_detailed.csv'.format(archive_folder_name))
    model_output_non_detailed.to_csv(config.root_dir + config.slash + '{}\\model_output_non_detailed.csv'.format(archive_folder_name))
    model_output_all_with_fuels.to_csv(config.root_dir + config.slash + '{}\\model_output_all_with_fuels.csv'.format(archive_folder_name))
    
    #save config file to folder
    shutil.copyfile(config.root_dir + config.slash + 'code\\config.py', '{}\\config.py'.format(archive_folder_name))

    #save all code files to folder, incmluding all subfolders (they are the code).
    recursively_save_file(config.root_dir + config.slash + 'code', archive_folder_name, 'file_extension=.py', exclude_archive_folder=True)
    #save all csvs in \input_data\user_input_spreadsheets
    recursively_save_file(config.root_dir + config.slash + 'input_data\\user_input_spreadsheets', archive_folder_name, file_extension='.csv', exclude_archive_folder=True)
    recursively_save_file(config.root_dir + config.slash + f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}', archive_folder_name, file_extension='.csv', exclude_archive_folder=True)
    recursively_save_file(config.root_dir + config.slash + 'output_data\\for_other_modellers', archive_folder_name, exclude_archive_folder=True)
    
    #and save individual files
    fuel_mixing_assumptions = config.root_dir + config.slash + 'input_data\\fuel_mixing_assumptions.xlsx'
    shutil.copyfile(fuel_mixing_assumptions, '{}\\fuel_mixing_assumptions.xlsx'.format(archive_folder_name))

    recursively_save_file(config.root_dir + config.slash + 'config\\concordances_and_config_data\\computer_generated_concordances', archive_folder_name, '.csv', exclude_archive_folder=True)

    # zip_up_folder(config, archive_folder_name)
...
def zip_up_folder(config, archive_folder_name):
    # if os.path.exists(config.root_dir + config.slash + 'C:\\Users\\finbar.maunsell\\Documents'):
    ...
    output_file = archive_folder_name +config.slash+ config.FILE_DATE_ID + '_0'
...
def save_economy_projections_and_all_inputs(config, ECONOMY_ID, ZIP_UP_ARCHIVE_FOLDER=True, ARCHIVED_FILE_DATE_ID=None, transport_data_system_FILE_DATE_ID_2=None):
    ...
    if transport_data_system_FILE_DATE_ID_2 is None:
        ...
        if not os.path.exists(config.root_dir + config.slash + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{ARCHIVED_FILE_DATE_ID}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
            ARCHIVED_FILE_DATE_ID_2 = utility_functions.get_latest_date_for_data_file(data_folder_path=config.root_dir + config.slash +'intermediate_data\\input_data_optimisations', file_name_start=f'optimised_data_{ECONOMY_ID}_', file_name_end=f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    ...
    #create folder:
    archive_folder_name = config.root_dir + config.slash + 'output_data\\archived_runs\\{}_{}'.format(ECONOMY_ID, ARCHIVED_FILE_DATE_ID) + '_' + datetime.datetime.now().strftime("%H%M") 
    ...
    if not os.path.exists(archive_folder_name):
        ...
        user_input = input('{} already exists. Do you want to continue? (y\\n)'.format(archive_folder_name))
        if user_input == 'y':
            pass
        else:
    ...
    for file in sometimes_files:
        if os.path.exists(config.root_dir + config.slash + file):
            files_list.append(config.root_dir + config.slash + file)
    ...
    for file in files_list:
        ...
        dotdot = False
        if config.slash in file:
            if file.startswith(config.root_dir + config.slash + '..\\transport_data_system'):
                file = file[3:]
                dotdot = True
            ...
            folder_structure = file.split(config.slash)[:-1]
            folder_structure = config.slash.join(folder_structure)
            if not os.path.exists(archive_folder_name+config.slash+folder_structure):
                os.makedirs(archive_folder_name+config.slash+folder_structure)
        
        if os.path.exists(archive_folder_name+config.slash+file):
            print(f'WARNING: {file} already exists in archive folder. Must already have been saved there')
            
        if dotdot:
            shutil.copyfile(config.root_dir + config.slash + '..\\' +file, archive_folder_name+config.slash+file)
        else:
            shutil.copyfile(config.root_dir + config.slash + file, archive_folder_name+config.slash+file)
        
    # #for inputs not dated with {FILE_DATE_ID} we will save the latest version of the file
    # energy_use_esto = pd.read_csv(config.root_dir + config.slash +f'input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv')
    date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'input_data\\9th_model_inputs', 'model_df_wide_')
    #create folder structure
    ...
    shutil.copyfile(config.root_dir + config.slash + f'input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv', archive_folder_name+f'\\input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv')
    
    macro_date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'input_data\\macro', 'APEC_GDP_data_')
    # macro_df_wide = pd.read_csv(config.root_dir + config.slash +f'input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv')
    #create folder structure
    if not os.path.exists(archive_folder_name+'\\input_data\\macro'):
        os.makedirs(archive_folder_name+'\\input_data\\macro')
    shutil.copyfile(config.root_dir + config.slash + f'input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv', archive_folder_name+f'\\input_data\\macro\\APEC_GDP_data_{macro_date_id}.csv')
    ...
    optimisation_data_date_id =utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'intermediate_data\\input_data_optimisations', f'optimised_data_{ECONOMY_ID}_', f'_{transport_data_system_FILE_DATE_ID_2}.pkl', EXCLUDE_DATE_STR_START=True)
    if not os.path.exists(config.root_dir + config.slash + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl'):
    ...
    shutil.copyfile(config.root_dir + config.slash + f'intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl', archive_folder_name+f'\\intermediate_data\\input_data_optimisations\\optimised_data_{ECONOMY_ID}_{optimisation_data_date_id}_{transport_data_system_FILE_DATE_ID_2}.pkl')
    
    #move the bunkers data
    date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'output_data\\for_other_modellers\\output_for_outlook_data_system', '{}_international_bunker_energy_use_'.format(ECONOMY_ID))
    ...
    shutil.copyfile(config.root_dir + config.slash + 'output_data\\for_other_modellers\\output_for_outlook_data_system\\{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id), archive_folder_name+'\\output_data\\for_other_modellers\\output_for_outlook_data_system\\{}_international_bunker_energy_use_{}.csv'.format(ECONOMY_ID,date_id))
    
    #do same thing but for international_bunker_energy_use_{date_id}.csv
    shutil.copyfile(config.root_dir + config.slash + 'output_data\\for_other_modellers\\output_for_outlook_data_system\\international_bunker_energy_use_{}.csv'.format(date_id), archive_folder_name+'\\output_data\\for_other_modellers\\output_for_outlook_data_system\\international_bunker_energy_use_{}.csv'.format(date_id))
    ...
    for file in os.listdir(config.root_dir + config.slash + 'output_data\\for_other_modellers\\{}'.format(ECONOMY_ID)):
        shutil.copyfile(config.root_dir + config.slash + 'output_data\\for_other_modellers\\{}\\'.format(ECONOMY_ID)+file, archive_folder_name+'\\output_data\\for_other_modellers\\{}\\'.format(ECONOMY_ID)+file)
    ...
    for file in os.listdir(config.root_dir + config.slash + 'input_data\\user_input_spreadsheets'):
        if file.endswith('.csv'):
            shutil.copyfile(config.root_dir + config.slash + 'input_data\\user_input_spreadsheets\\'+file, archive_folder_name+'\\input_data\\user_input_spreadsheets\\'+file)
    ...
    if os.path.exists(config.root_dir + config.slash + 'intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID)):
        ...
        shutil.copyfile(config.root_dir + config.slash + 'intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID), archive_folder_name+ '\\intermediate_data\\road_model\\change_dataframe_aggregation_{}.csv'.format(ECONOMY_ID))
    ...
    for file in os.listdir(config.root_dir + config.slash + f'plotting_output\\dashboards\\{ECONOMY_ID}'):
        if file.endswith('.html'):
            shutil.copyfile(config.root_dir + config.slash + f'plotting_output\\dashboards\\{ECONOMY_ID}\\'+file, archive_folder_name+f'\\plotting_output\\dashboards\\{ECONOMY_ID}\\'+file)              
                
    #save entire code folder
    recursively_save_file(config.root_dir + config.slash + 'code', archive_folder_name+'\\code', file_extension='.py', exclude_archive_folder=True, keep_folder_structure=True) 
    ...
    for file in os.listdir(config.root_dir + config.slash + 'config\\concordances_and_config_data'):
        if file.endswith('.csv'):
            shutil.copyfile(config.root_dir + config.slash + 'config\\concordances_and_config_data\\'+file, archive_folder_name+'\\config\\concordances_and_config_data\\'+file)
    ...
    # # input_data_based_on_previous_model_run.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_outputs\\{}_input_data_based_on_previous_model_run_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    # # energy_use_output.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_outputs\\{}_energy_use_output_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    #     demand_side_fuel_mixing = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))        
    ...
    # road_model_input_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    # non_road_model_input_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_non_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    # growth_forecasts_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    
    # #     vehicles_per_stock_parameters.to_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_vehicles_per_stock_parameters.csv'.format(ECONOMY_ID), index=False)
    #find latest version of vehicles_per_stock_parameters
    # #save parameters_estimates. at the very elast we will plot these later
    # parameters_estimates.to_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_parameters_estimates_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID), index=False)
    # gompertz_parameters = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_stocks_per_capita_threshold.csv'.format(config.FILE_DATE_ID,ECONOMY_ID))
    ...
    #     growth_forecasts = pd.read_pickle(config.root_dir + config.slash + f'intermediate_data\\road_model\\{ECONOMY_ID}_final_road_growth_forecasts.pkl')
    # else:
    #     growth_forecasts = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}\\{ECONOMY_ID}_growth_forecasts_wide.csv')
    # #load all other data
    # non_road_model_input = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}\\{ECONOMY_ID}_non_road_model_input_wide.csv')
    # output_file_name = 'intermediate_data\\non_road_model\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name)
    # road_model_input = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # growth_forecasts = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    ...
    # supply_side_fuel_mixing.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    # transport_data_system_df = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\transport_data_system_extract.csv')
    # #save data so it can be used for plotting and such:
    # new_sales_shares_all_new.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_vehicle_sales_share.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index = False)
    # #save data without the new drive cols for non road
    # model_output_detailed.to_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False
    # model_output_non_detailed.to_csv(config.root_dir + config.slash + 'output_data\\model_output\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False
    # model_output_all_with_fuels.to_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_detailed.to_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_non_detailed.to_csv(config.root_dir + config.slash + 'output_data\\model_output\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # model_output_all_with_fuels.to_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name), index=False)
    # road_model_output = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    # model_output_all_with_fuels = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    ...
    #     model_output_with_fuels_ = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     model_output_detailed_ = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     energy_output_for_outlook_data_system_ = pd.read_csv(config.root_dir + config.slash +f'output_data\\for_other_modellers\\output_for_outlook_data_system\\{ECONOMY_ID}_{config.FILE_DATE_ID}_transport_energy_use.csv')
    #     chargers_ = pd.read_csv(config.root_dir + config.slash + 'output_data\\for_other_modellers\\charging\\{}_estimated_number_of_chargers.csv'format(ECONOMY_ID))
    #     supply_side_fuel_mixing_ = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     road_model_input_ = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     model_output_detailed_detailed_non_road_drives_ = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     growth_forecasts_ = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\model_inputs\\{config.FILE_DATE_ID}\\{ECONOMY_ID}_growth_forecasts_wide.csv')
    #     first_road_model_run_data_ = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\first_run_{}_{}'.format(ECONOMY_ID, config.model_output_file_name))
    #     new_sales_shares_all_plot_drive_shares_ = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_vehicle_sales_share.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    #     gompertz_parameters_df_ = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\road_model\\{}_parameters_estimates_{}.csv'.format(ECONOMY_ID, config.FILE_DATE_ID))
    # original_model_output_8th = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\reformatted\\activity_energy_road_stocks.csv').rename(columns={'Year':'Date'})
    # emissions_factors = pd.read_csv(config.root_dir + config.slash + 'config\\9th_edition_emissions_factors.csv')
    # date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'input_data\\9th_model_inputs', 'model_df_wide_')
    # energy_use_esto = pd.read_csv(config.root_dir + config.slash +f'input_data\\9th_model_inputs\\model_df_wide_{date_id}.csv')
    # data_8th = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\reformatted\\activity_energy_road_stocks.csv')
    # energy_8th = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\reformatted\\8th_energy_by_fuel.csv')
    #     all_data = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output\\{}_{}'.format(ECONOMY_ID,config.model_output_file_name))
    # new_transport_dataset = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\transport_data_system_extract.csv')
    # user_input = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\model_inputs\\{ECONOMY_ID}_user_inputs_and_growth_rates.csv')
    # growth = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\regression_based_growth_estimates.csv')
    # #save data    
    # road_model_input_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    # non_road_model_input_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_non_road_model_input_wide.csv'.format(config.FILE_DATE_ID,ECONOMY_ID), index=False)
    # growth_forecasts_wide.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID,ECONOMY_ID), index=False)
    # stocks_per_capita_threshold.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_stocks_per_capita_threshold.csv'.format(config.FILE_DATE_ID,ECONOMY_ID), index=False)
    # #lastly resave these files before they get changed in the modelling process
    # supply_side_fuel_mixing = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID,ECONOMY_ID))
    # demand_side_fuel_mixing = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # supply_side_fuel_mixing.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    # demand_side_fuel_mixing.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID), index=False)
    ...
    # supply_side_fuel_mixing = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_supply_side_fuel_mixing.csv'.format(config.FILE_DATE_ID,ECONOMY_ID))
    # demand_side_fuel_mixing = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_demand_side_fuel_mixing.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # road_model_input_wide = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # non_road_model_input_wide = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_non_road_model_input_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # growth_forecasts_wide = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\{}\\{}_aggregated_growth_forecasts_wide.csv'.format(config.FILE_DATE_ID, ECONOMY_ID))
    # macro2.to_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\regression_based_growth_estimates.csv', index=False)
    # transport_data_system_folder = '..\\transport_data_system'
    # transport_data_system_df = pd.read_csv(config.root_dir + config.slash + '{}\\output_data\\combined_data_{}.csv'.format(transport_data_system_folder,config.transport_data_system_FILE_DATE_ID))
...
def revert_to_previous_version_of_files(config, economy, archive_folder_name, UNZIP_ARCHIVE_FOLDER=True, CURRENT_FILE_DATE_ID=None):
    ...
    save_economy_projections_and_all_inputs(config, economy, ARCHIVED_FILE_DATE_ID=CURRENT_FILE_DATE_ID)#save current state of files
    ...
    breakpoint()#NOTE AFTER CREATING config.root_dir + config.slash + FUNCTIONALITY, I DONT KNOW IF THIS IS WORKING SO IF SET IT TO RAISE AN ERROR
    ...
    for root, dirs, files in os.walk(archive_folder_name):
        for file in files:
            if file.endswith('.py') or file.endswith('.yaml') or file.endswith('.yaml') or file.endswith('.csv') or file.endswith('.xlsx') or file.endswith('.pkl') or file.endswith('.html'):
                ...
                root = root.replace(archive_folder_name, config.root_dir + config.slash + '')
                root = root.strip(config.slash)
                
                if root.startswith('code\\utility_functions'):
                    continue
                if root.startswith(config.root_dir + config.slash +'transport_data_system'):
                    #check folder exists
                    if not os.path.exists(config.root_dir + config.slash + '..\\'+root):
                        os.makedirs('..\\'+root)
                    #save file in transport_data_system folder
                    shutil.copyfile(root+config.slash+file, '..\\'+root+config.slash+file)
                else:
                    #check folder exists
                    if not os.path.exists(root):
                        os.makedirs(root)
                    #save file in the same place it came from
                    shutil.copyfile(archive_folder_name +config.slash+ root+config.slash+file, root+config.slash+file)
...
# copy_folder_structure(config, source_folder, destination_folder)
...
# remove_gitkeep_files(config, folder)
#%%
