...
from ..calculation_functions import logistic_fitting_functions
#################
...
def plot_logistic_fit(config, date, stocks_per_capita, gdp_per_capita, gamma, growth_rate, midpoint, economy_ttype_scenario, show_plots, matplotlib_bool, plotly_bool):
    if plotly_bool:
        ...
        write_to_img = False
        if write_to_img:
            fig.write_image(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\fitting\\log_fit_{economy_ttype_scenario}.png')
        #write to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\fitting\\log_fit_{economy_ttype_scenario}.html')
        ...
        if write_to_img:
            fig.write_image(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\fitting\\log_fit_gdp_per_capita_{economy_ttype_scenario}.png')
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\fitting\\logistic_fit_gdp_per_capita_{economy_ttype_scenario}.html')
...
def plot_logistic_function_all_economies(config, model_data_logistic_predictions, activity_growth_estimates, parameters_estimates, new_model_data, show_plots, matplotlib_bool, plotly_bool, FIT_LOGISTIC_CURVE_TO_DATA):
    ...
    if plotly_bool:
        ...
        if all_data_stocks_per_capita.empty:
            ...
            #write to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\log_fit_new_stocks_per_capita_all_economies.html')
            ######################
            #and plot the same but wth gdp per capita in x
            ...
            #write to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\log_fit_new_stocks_per_capita_gdp_per_capita_all_economies.html')
        ...
        if all_data_stocks.empty:
            ...
            #write to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\log_fit_new_stocks_all_economies.html')
        ...
        if all_data_activity_growth.empty:
            ...
            #write to html
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\log_fit_new_activity_growth.html')
        
        ######################

def plot_logistic_function_by_economy(config, model_data_logistic_predictions, activity_growth_estimates, parameters_estimates, new_model_data, show_plots, matplotlib_bool, plotly_bool, FIT_LOGISTIC_CURVE_TO_DATA):
    #we will plot the results of the logistic function fit. To cut down on the number of plots we will facet by economy and onily plot the first scenario
    #we will also plot the gamma value for each economy as a horizontal line if the data is on stock per capita
        ...
        if all_data.empty:
            ...
            for economy in all_data.Economy.unique():
                ...
                if all_data_stocks_per_capita.empty:
                    ...
                    #write to html
                    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\economy\\log_fit_new_stocks_per_capita_{economy}.html')
                    ...
                    #write to html
                    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\economy\\log_fit_new_stocks_per_capita_gdp_per_capita_{economy}.html')
                ...
                if all_data_activity_growth.empty:
                    ...
                    #write to html
                    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\economy\\log_fit_new_activity_growth_{economy}.html')
                ...
                fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\economy\\log_fit_new_stocks_{economy}.html')
...
def plot_aggregated_input_data_for_logisitc_fitting(config, ECONOMY_ID, new_model_data):
    ...
    #plot the data for ['Economy', 'Scenario', 'Date', 'Transport Type']
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_stocks.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Activity', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_activity.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Travel_km', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_travel_km.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Mileage', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_mileage.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Occupancy_or_load', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_occupancy_or_load.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Population', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_population.html'.format(ECONOMY_ID))
    
    fig = px.line(a, x='Date', y='Gdp_per_capita', color='Transport Type')
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\input_exploration\\gompertz\\aggregated_input_data\\{}_gdp_per_capita.html'.format(ECONOMY_ID))
