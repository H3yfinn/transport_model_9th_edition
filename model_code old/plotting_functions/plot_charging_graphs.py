...
def plot_required_chargers(config, ECONOMY_ID):
    ...
    total_kwh_of_battery_capacity = pd.read_csv(config.root_dir + config.slash +f'output_data\\for_other_modellers\\charging\\{ECONOMY_ID}_estimated_number_of_chargers.csv')
    ...
    for economy in df['Economy'].unique():
        for scenario in df['Scenario'].unique():
            ...
            fig.write_html(config.root_dir + config.slash +f'plotting_output\\charging_requirements\\{title}.html')
...
def plot_required_evs(config, ev_stocks_and_chargers, colors_dict, economy, date, scenario):
