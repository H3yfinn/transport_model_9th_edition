...
def produce_lots_of_LMDI_charts(config, ECONOMY_ID, USE_LIST_OF_CHARTS_TO_PRODUCE = False, PLOTTING = False, USE_LIST_OF_DATASETS_TO_PRODUCE=True, END_DATE=2070, PLOT_EFFECT_OF_ELEC_EMISSIONS_FACTOR=True, INCLUDE_LIFECYCLE_EMISSIONS=True, PLOT_EMISSIONS_FACTORS=False, SET_START_DATE_TO_AFTER_COVID=True):
    ...
    if ECONOMY_ID == 'all':
        all_data = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output\\all_economies_{}_{}'.format(config.FILE_DATE_ID, config.model_output_file_name))
        energy_use_by_fuels = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\all_economies_{}_{}'.format(config.FILE_DATE_ID, config.model_output_file_name))
        detailed_data = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\all_economies_{}_{}'.format(config.FILE_DATE_ID, config.model_output_file_name))
    else:
        ...
        energy_use_by_fuels = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_{}'.format(ECONOMY_ID,config.model_output_file_name))
        detailed_data = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_{}'.format(ECONOMY_ID,config.model_output_file_name))
    ...
    for combination_dict in combination_dict_list:
        ...
        try:
            ...
            output_data_folder=config.root_dir + config.slash + combination_dict['output_data_folder']
            plotting_output_folder=config.root_dir + config.slash + combination_dict['plotting_output_folder']
...
def calculate_emissions_effect_from_additive_data(config,ECONOMY_ID, extra_identifier, output_data_folder, energy_use_by_fuels, all_data, combination_dict, PLOT_EMISSIONS_FACTORS):
    ...
    data.to_csv(f'{output_data_folder}\\{extra_identifier2}_additive.csv', index=False)
...
def plot_lca_bars(config, lca, numeric_cols=[], SIMPLE=True):
    if SIMPLE:
        ...
        fig.write_html(config.root_dir + config.slash + f'plotting_output\\lifecycle_emissions\\lifecycle_emissions_by_source_SIMPLE.html')
        ...
        #make text bigger
        fig.update_layout(font_size=35)
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\lifecycle_emissions\\lifecycle_emissions_SIMPLE.html')
        ...
        #make text bigger
        fig.update_layout(font_size=35)
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\lifecycle_emissions\\lifecycle_emissions_by_source_FACETED.html')  
...
def plot_lifecycle_emissions(config, emissions_factors, fuels_to_plot):
    ...
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\lifecycle_emissions\\{economy}_lifecycle_emissions_LINE.html')
