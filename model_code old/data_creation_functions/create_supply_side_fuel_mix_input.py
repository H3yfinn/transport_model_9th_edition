...
def create_supply_side_fuel_mixing_input(config, ECONOMY_ID, X_ORDER='linear', AUTO_OPEN=False, PLOT=False):
    ...
    mixing_assumptions = pd.read_excel(config.root_dir + config.slash + 'input_data\\fuel_mixing_assumptions.xlsx',sheet_name='supply_side')
    #drop comment col
    mixing_assumptions.drop(columns=['Comment'], inplace=True)
    #cols Region	Fuel	New_fuel	Date	Reference	Target

    regions = pd.read_excel(config.root_dir + config.slash + 'input_data\\fuel_mixing_assumptions.xlsx',sheet_name='regions')
    ...
    shutil.copy(config.root_dir + config.slash +  'input_data\\fuel_mixing_assumptions.xlsx', archiving_folder + '\\fuel_mixing_assumptions.xlsx')
