...
def plot_and_compare_new_growth_coefficients(config, GROWTH_MEASURES_TO_PLOT, ACTIVITY_MEASURES_TO_PLOT):
    #LOAD DATA:
    with open(config.root_dir + config.slash + 'intermediate_data\\growth_analysis\\measures_to_plot.txt', 'r') as f:
        measures_to_plot = [line.rstrip() for line in f]
        
    with open(config.root_dir + config.slash + 'intermediate_data\\growth_analysis\\indexed_measures_to_plot.txt', 'r') as f:
        indexed_measures_to_plot = [line.rstrip() for line in f]
    df = pd.read_pickle(config.root_dir + config.slash + 'intermediate_data\\growth_analysis\\df_growth_parameter_analysis.pkl')
    ...
    for economy in df['Economy'].unique():
        ...
        #write to html
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\growth_analysis\\{title}.html')
...
def plot_growth_coefficients(config, growth_coefficients_df, independent_variables):
    ...
    for model in growth_coefficients_df.Model.unique():
        ...
        #save
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\growth_analysis\\{model}_growth_coefficients.html')
