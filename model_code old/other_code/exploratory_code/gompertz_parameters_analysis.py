...
if test_params:
    ...
    # Show the plot
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\gompertz_curve_test_{gamma}.html', auto_open=True)
#%%
...
if analyse_this:
    ...
    for i, gamma in enumerate(gamma_values):
        ...
        # Show the plot
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\01aus_gompertz_curve_test_{gamma}.html', auto_open=True)
#%%
...
if analyse_this:
    ...
    # save plot as html
    fig.write_html(config.root_dir + config.slash +f'plotting_output\\input_exploration\\gompertz\\01aus_gompertz_curve_log_fit.html', auto_open=True)
...
if run_this_section == True:
    ...
    #calculate stocks per capita (stocks are currently in millions and population in thousands, as determined by code\grooming_code\2_aggregate_data_for_model.py line 79)
