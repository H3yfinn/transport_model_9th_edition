...
macro = pd.read_csv(config.root_dir + config.slash + 'D:\\APERC\\transport_model_9th_edition\\input_data\\macro\\APEC_GDP_population.csv')
#pull in activity_growth 
activity_growth = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\activity_growth.csv')
...
#load it in
energy_use = pd.read_csv(config.root_dir + config.slash + 'D:\\APERC\\transport_data_system\\intermediate_data\\EGEDA\\EGEDA_transport_outputDATE20230215.csv')
...
def regression_analysis(config, df, x, y, drop_outliers=False):
    #drop rows with na
    df = df.dropna()
...
import plotly.graph_objects as go
def plot_regression_results(config, results, model, x_values, y_values, df, x, y, plot_name, add_one=True, log_x=False):
    ...
    if regression_type == 'multiple':
        ...
        #save the plot
        fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\{}.html'.format(plot_name))
        return
...
if analyse:
    ...
    for economy in df.economy.unique():
        ...
        plot_regression_results(config, regression_analysis(economy_df, independent_variables, dependent_variable), economy_df, independent_variables, dependent_variable, '{}_regression_macro_growth_vs_energy_total_growth'.format(economy))
        
        independent_variables = ['gdp_per_capita', 'gdp_times_capita', 'gdp', 'population']
        dependent_variable = 'energy_total'

        plot_regression_results(config, regression_analysis(economy_df, independent_variables, dependent_variable), economy_df, independent_variables, dependent_variable, '{}_regression_macro_vs_energy_total'.format(economy))

        independent_variables = ['gdp_per_capita_growth', 'gdp_times_capita_growth', 'gdp_growth']
        dependent_variable = 'energy_total_growth'

        plot_regression_results(config, regression_analysis(economy_df, independent_variables, dependent_variable), economy_df, independent_variables, dependent_variable, '{}_regression_macro_growth_vs_energy_total_growth_no_pop'.format(economy))
...
for economy in df.economy.unique():
    ...
    independent_variables = ['gdp_per_capita_growth', 'gdp_times_capita_growth', 'gdp_growth']
    dependent_variable = 'energy_total_growth'
    results,model, x_values, y_values = regression_analysis(config, economy_df, independent_variables, dependent_variable, drop_outliers=True)
    plot_regression_results(config, results,model, x_values, y_values, economy_df, independent_variables, dependent_variable, '{}_regression_macro_growth_vs_energy_total_growth_no_pop'.format(economy))
...
# %%
#save to csv
growth_coefficients_df.to_csv(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\growth_coefficients.csv')
...
#save
fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\growth_coefficients.html')
...
if plot_this:
    ...
    # fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\activity_8th_vs_regression_growth_indexes.html')
    ...
    #save to plotting_output\growth_analysis
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\activity_8th_vs_regression_growth.html')
#%%
# %%
...
if analyse == True:
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\energy_vs_gdp_per_capita.html', auto_open=True)
    ...
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\energy_vs_gdp_per_capita_index.html', auto_open=True)
...
regional_mapping = pd.read_csv(config.root_dir + config.slash + 'config\\concordances_and_config_data\\region_economy_mapping.csv')
...
if independent_variables == ['gdp_per_capita_growth', 'gdp_times_capita_growth', 'gdp_growth']:
    ...
    #save to csv
    growth_coefficients_df.to_csv(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\growth_coefficients_by_region.csv')
    #plot all growth coefficients on a bar chart with facets for economys and x as the coefficients
    #melt
    ...
    #save
    fig.write_html(config.root_dir + config.slash + 'plotting_output\\growth_analysis\\macro_regression_final\\growth_coefficients_by_region.html')
    ...
    for economy in df.region.unique():
        ...
        dependent_variable = 'energy_total_growth'
        results,model, x_values, y_values = regression_analysis(config, economy_df, independent_variables, dependent_variable, drop_outliers=False)
        plot_regression_results(config, results,model, x_values, y_values, economy_df, independent_variables, dependent_variable, '{}_regression_macro_growth_vs_energy_total_growth_no_pop'.format(economy))
        #extract coefficients from model
        coefficients = results.params
        #save coefficients to df
...
#resave
growth_coefficients_df.to_csv(config.root_dir + config.slash + 'input_data\\growth_coefficients_by_region.csv', index=False)
# %%
