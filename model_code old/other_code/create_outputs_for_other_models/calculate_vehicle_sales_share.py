...
model_output_8th_with_sales_share.to_csv(config.root_dir + config.slash + 'intermediate_data\\cleaned_input_data\\8th_activity_efficiency_energy_road_stocks_sales_share.csv', index=False)
...
for transport_type in model_output_8th_with_sales_share['Transport Type'].unique():
    ...
    for scenario in model_output_8th_with_sales_share['Scenario'].unique():
        ...
        plotly.offline.plot(fig, filename=config.root_dir + config.slash + 'plotting_output\\8th_edition\\' + title + '.html', auto_open=False)
# %%

#compare agaisnt previously calcualted vlaues: 
prev_sales_share = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\cleaned_input_data\\vehicle_sales_share.csv')
...
for transport_type in model_output_8th_with_sales_share_diff['Transport Type'].unique():
    ...
    for scenario in model_output_8th_with_sales_share_diff['Scenario'].unique():
        ...
        plotly.offline.plot(fig, filename=config.root_dir + config.slash + 'plotting_output\\8th_edition\\archive\\' + title + '.html', auto_open=AUTO_OPEN_PLOTLY_GRAPHS)
#%%
