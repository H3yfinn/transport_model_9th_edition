...
def create_output_for_outlook_data_system(config, ECONOMY_ID, model_output_file_name=None, FILE_DATE_ID=None):
    ...
    model_output_all_with_fuels_df = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, model_output_file_name))
    stocks_df = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, model_output_file_name))[['Date', 'Economy', 'Vehicle Type', 'Medium', 'Transport Type', 'Drive', 'Scenario', 'Stocks']]
    activity_df = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_detailed\\{}_NON_ROAD_DETAILED_{}'.format(ECONOMY_ID, model_output_file_name))[['Date', 'Economy', 'Vehicle Type', 'Medium', 'Transport Type', 'Drive', 'Scenario', 'Activity']]
    ...
    for dataset in outputs_dict.keys():
        ...
        new_final_df.to_csv(config.root_dir + config.slash + f'output_data\\for_other_modellers\\output_for_outlook_data_system\\{ECONOMY_ID}_{FILE_DATE_ID}_{output_file_name}.csv', index=False)
...
def concatenate_outlook_data_system_outputs(config):
    ...
    for file_ending in ['transport_energy_use', 'transport_stocks']:
        ...
        for file in os.listdir(config.root_dir + config.slash + 'output_data\\for_other_modellers\\output_for_outlook_data_system'):
            if file.endswith('_{}_{}.csv'.format(config.FILE_DATE_ID, file_ending)):
                ...
                if file == '{}_{}.csv'.format(config.FILE_DATE_ID, file_ending):
                    continue
                df = pd.read_csv(config.root_dir + config.slash +f'output_data\\for_other_modellers\\output_for_outlook_data_system\\{file}')
        ...
        #save the final df:
        final_df.to_csv(config.root_dir + config.slash + f'output_data\\for_other_modellers\\output_for_outlook_data_system\\{config.FILE_DATE_ID}_{file_ending}.csv', index=False)
