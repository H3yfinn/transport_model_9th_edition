...
def compare_esto_energy_to_data(config):
    ...
    # energy_use_esto = pd.read_csv(config.root_dir + config.slash +f'..\\transport_data_system\\intermediate_data\\EGEDA\\model_input_9th_cleanedDATE{date_id}.csv')


    # original_model_output_with_fuels = pd.read_csv(config.root_dir + config.slash + 'output_data\\model_output_with_fuels\\{}'.format(config.model_output_file_name))#cols 	Date	Economy	Scenario	Transport Type	Vehicle Type	Drive	Medium	Fuel	Energy
    ...
    for economy in original_model_output_with_fuels['Economy'].unique():
        ...
        fig.write_html(config.root_dir + config.slash +f'plotting_output\\diagnostics\\esto_vs_model_energy_use\\{title}.html')
