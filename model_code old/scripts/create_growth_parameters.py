...
date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'input_data\\macro\\', 'APEC_GDP_data_')
macro = pd.read_csv(config.root_dir + config.slash +f'input_data\\macro\\APEC_GDP_data_{date_id}.csv')
#pull in activity_growth from 8th edition
activity_8th = pd.read_csv(config.root_dir + config.slash + 'input_data\\from_8th\\reformatted\\8th_activity_efficiency_energy_road_stocks_sales_share.csv')
...
date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + '..\transport_data_system\intermediate_data\EGEDA\', 'EGEDA_transport_output')
energy_use = pd.read_csv(config.root_dir + config.slash +f'..\\transport_data_system\\intermediate_data\\EGEDA\\EGEDA_transport_outputDATE{date_id}.csv')

activity_9th = pd.read_csv(config.root_dir + config.slash + 'intermediate_data\\model_inputs\\transport_data_system_extract.csv')
...
def calculate_and_analyse_activity_growth_using_new_growth_coefficients(config, growth_coefficients_df, energy_macro, activity_9th, activity_8th, independent_variables, models, GROWTH_MEASURES_TO_PLOT, ACTIVITY_MEASURES_TO_PLOT):
    ...
    with open(config.root_dir + config.slash + 'intermediate_data\\growth_analysis\\measures_to_plot.txt', 'w') as f:
        for item in measures_to_plot:
            f.write("%s\n" % item)
    with open(config.root_dir + config.slash + 'intermediate_data\\growth_analysis\\indexed_measures_to_plot.txt', 'w') as f:
...
def choose_and_filter_for_model_by_region(config, chosen_model_by_region_dict, chosen_file_date_id=None, region_column = 'Region_growth_regression2'):
    ...
    if chosen_file_date_id is not None:
        ...
        date_id = utility_functions.get_latest_date_for_data_file(config.root_dir + config.slash + 'intermediate_data\growth_analysis\','growth_coefficients_by_region')
        
    growth_coefficients_df = pd.read_csv(config.root_dir + config.slash +f'intermediate_data\\growth_analysis\\growth_coefficients_by_region{date_id}.csv')
    ...
    df.to_csv(config.root_dir + config.slash + 'input_data\\growth_coefficients_by_region.csv', index=False)
