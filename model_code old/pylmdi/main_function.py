...
def run_divisia(config, data_title, extra_identifier, activity_data, energy_data, structure_variables_list, activity_variable = 'Activity', emissions_variable = 'Emissions', energy_variable = 'Energy', emissions_divisia = False, emissions_data=[], time_variable='Year', hierarchical=False, output_data_folder='output_data'):
    ...
    ######################################################################
        ...
        total_activity = activity_data.groupby(time_variable).sum(numeric_only=True).reset_index()
        total_activity = total_activity.rename(columns={activity_variable:'Total_{}'.format(activity_variable)})
        # hierarchical_additive_output = pd.merge(hierarchical_additive_output,total_energy,on=[time_variable], how='left')
        # hierarchical_additive_output = pd.merge(hierarchical_additive_output,total_activity,on=[time_variable], how='left')
        hierarchical_multiplicative_output = pd.merge(hierarchical_multiplicative_output,total_energy,on=[time_variable], how='left')
        ...
        total_activity = activity_data.groupby(time_variable)[activity_variable].sum().reset_index()
        total_activity = total_activity.rename(columns={activity_variable:'Total_{}'.format(activity_variable)})
        # hierarchical_additive_output = pd.merge(hierarchical_additive_output,total_emissions,on=[time_variable], how='left')
        # hierarchical_additive_output = pd.merge(hierarchical_additive_output,total_activity,on=[time_variable], how='left')
        hierarchical_multiplicative_output = pd.merge(hierarchical_multiplicative_output,total_emissions,on=[time_variable], how='left')
        hierarchical_multiplicative_output = pd.merge(hierarchical_multiplicative_output,total_activity,on=[time_variable], how='left')
